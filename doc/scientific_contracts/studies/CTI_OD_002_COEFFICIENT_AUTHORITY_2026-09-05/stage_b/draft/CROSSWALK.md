...
| UFC-REQ-002 | `equations.tex`, Occurrence index and broadcast | First verify the unique target population, payload-publication identity, and target broadcast binding; then independently compare the stored membership manifest with that target. Row/shape order cannot substitute. Predictions: UFC-PRED-001, 007, 009. |
...
| UFC-REQ-006 | `equations.tex`, Fixed constant and normalization | Accept singleton support; fail structural establishment on an empty target or duplicate, ambiguous, or conflicting target identity without repair. Test stored membership mismatch only after a unique target exists. Predictions: UFC-PRED-002, 003, 007, 009. |
...
| UFC-REQ-012 | `definitions.tex`, Identity, use, object, applicability, inputs, and restrictions | Check every required Registry field, exact input, the disjoint structural/fidelity truth-domain rule, missing behavior, no-exception declaration, and lineage field. Predictions: UFC-PRED-007--009. |
| UFC-REQ-013 | `definitions.tex`, Profile inputs; `assumptions.tex`, decision semantics | Exercise structural target identity, stored membership, payload availability/fidelity, sample validity, retention, consumer admission, and numerical contribution as distinct facts. Predictions: UFC-PRED-007--010. |
...
| UFC-REQ-015 | `assumptions.tex`, Knowledge and four axes | Verify independent R/A/E/Z storage, the uninstantiated eligibility state, and `Z=realized, E=ineligible` for a completed false predicate; reserve failed Z for profile-artifact failure. Prediction: UFC-PRED-007. |
...
| UFC-REQ-017 | `assumptions.tex`, Payload order and failure scope | Verify structural missing/conflict stops the question; unexplained unreadability or absent fidelity evidence is unknown; authoritative corruption and missing/extra/repeated stored membership are false. Preserve conflict, zero, non-finite, unrepresentable, and all stated scopes. Predictions: UFC-PRED-003, 007--009. |
