...
Draft authority: **SCI-PTC-COEFFICIENT-UNIFORM v0.1-draft.1 / r0.4**  
Revision date: **2026-09-06**  
Status: **exact author proposal; inactive and unavailable for use**

Revision r0.4 narrows the structural category to authoritative
identity/binding-record defects. Integrity of bytes in an already bound payload
continues to use Q_integrity F/U/C. This correction is within owner review
round 1 and changes no proposal state.
...
| Source identity | SCI-PTC-COEFFICIENT-UNIFORM v0.1-draft.1 / r0.4, bound by exact promoted hashes before activation |
...
| Compatibility/supersession | No alias or retroactive change. A wrong pair in one request is local; a corrupt/misbound authoritative q-to-g identity/binding record is shared. Changed rules create versioned successors; changed actual inputs create new realizations. |
...
| Source | SCI-PTC-COEFFICIENT-UNIFORM v0.1-draft.1 / r0.4; activation requires exact promoted source digest. |
...
| Structural gate | Exact active Registry/source/owner, application/CAL, observation, occurrence, q, authoritative q-to-g, family, selection-record, R_q, payload-publication, target-broadcast, profile, named consumer, admitted VAL structural scope/influence, and immutable provenance identity/binding records. A missing, conflicting, corrupt, or misbound identity/binding record makes the question unavailable. Bound payload-byte integrity is not part of this structural classification. Permission evidence is assessed separately by UQ-06. |
...
| Failure scope | Malformed occurrence or wrong otherwise-authoritative q,g request is local. A corrupt/misbound shared authoritative identity/binding record affects all references. After valid binding, corrupt/digest-failing/decoder-invalid payload bytes remain Q_integrity F; unavailable evidence is U and contradictory integrity evidence is C. Payload F affects that payload's handoffs. Permission affects one named consumer. Every cause is retained. |
...
Proposed identity: **SCI-PTC_TO_SCI-MAP_BOUNDARY v0.2-draft.1/r0.4**.
...
Proposed identity: **SCI-PTC_TO_SCI-JINC_BOUNDARY v0.2-draft.1/r0.4**.
