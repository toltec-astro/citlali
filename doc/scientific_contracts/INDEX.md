...
coordinate/projection boundaries. The later MAP r0.6 targeted closure selects
the initial one-hot projection, types contribution gates, places unique
originals by their own AST coordinates, and separates fixed-state from
procedure response while preserving exact source authority.
...
| [SCI-MAP — ordinary mapmaking and observation coaddition](packages/SCI-MAP/v0.1/README.md) | Owner-approved v0.1 scope; targeted scientific closure rendered `2026-08-27` | Formal, rationale, and ECS synchronized at r0.6; byte-identical PTC/MAP boundary, source-current atomic and aggregate VAL profiles, typed gates, own-coordinate exposure, and separated response roles; 52 requirement and 25 prediction IDs retained; authority not frozen | Hard numerical gates: exact PTC MAP-facing coefficient family and admitted `coverage_cut` numerical domain. Eight MAP owner decisions remain open; OD-008 is resolved | Scientific-owner content review of r0.6; no conformity, validation, achieved response/performance, freeze, readiness, or production claim |
