...
| [SCI-FLT — map-domain filtering tranche](packages/SCI-FLT/v0.1/README.md) | Recovery-first Stage A packet complete on `2026-08-30`; deterministic convolution/fixed low-pass separated from Wiener, matched/source-sensitive, and other inference-bearing families; internal dossier quarantined | Sanitized Scope Brief, authority boundaries, recovered deterministic-transform extract, typed operator/product taxonomy, ownership classification, eight-question owner ledger, and exact proposed seven-object future author set; Stage B not authorized | `FLT-ODQ-101--108` are open, beginning with whether to split provisional `SCI-FLT-DET` from the `SCI-FLT-INF` tranche before Stage B | Conduct the owner walkthrough starting at FLT-ODQ-101; do not launch Stage B or infer conformity, validation, calibration, performance, readiness, production, or freeze |
