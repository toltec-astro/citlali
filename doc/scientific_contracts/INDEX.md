...
| [SCI-FLT-INF — inference-bearing filtering holding study](studies/SCI-FLT-INF_STAGE_A_2026-08-30/README.md) | Recovery-first Stage A study complete for owner walkthrough; explicitly not a package or combined contract; ODQ-001 approved and closed `2026-08-30` | Historical full path is owner-selected as an optimal matched-template amplitude estimator with amplitude-unbiased normalization for a matching supplied template under declared assumptions; point-source and general-template cases are explicit; posterior/Wiener sky reconstruction and ordinary source-shaped convolution are distinct; exact frozen-NOI binding; protected SCI-FLT-FIXED bytes unchanged | `SCI-FLT-INF-ODQ-002`: package ownership and FLT/SRC split; later parent/operator/state/product decisions depend on it | Resolve ODQ-002 next; then create a separate package-local Stage A packet for the matched-template amplitude field; no Stage B launch or algorithm/conformity/validation claim |
