...
| [SCI-NOI — realization ensembles, empirical uncertainty, and standardized signal](packages/SCI-NOI/v0.1/README.md) | Final Stage A owner-review closure candidate under one-at-a-time review on `2026-08-29`; ODQ-101/102A/102B/102C/103/104 approved; ODQ-102D author-delegated | Fixed-state conditional-sign ordinary family; selected NOI-owned PTC-to-frozen-MAP route with optional inline MAP application and NOI realization-map output; one assignment per observation-scoped detector applies to all its admitted samples; coefficient balance is enforced separately within networks under a complement-symmetric randomized family; randomization intends source suppression but does not by construction establish source-free maps, with exact terminology delegated to Stage B; relearned methods require complete consequential-state identity but not exhaustive implementation provenance; exact finite mechanics belong to the scientific-contract author and remain unavailable pending later acceptance and coefficient/`coverage_cut` gates; Stage B not launched | `SCI-NOI-ODQ-105A` next: enabled, disabled, and partial-completion behavior; later decisions and exact final packet/profile/source approval remain open | Walk decisions one at a time from ODQ-105A, then complete exact profile/Registry/source bindings before conditional implementation-blind Stage B dispatch |
