...
| [SCI-FLT-MATCHED — optimal matched-template map filtering](packages/SCI-FLT-MATCHED/v0.1/README.md) | Package-local recovery, sanitized scope, owner-decision extract, boundaries, taxonomy, and six option assignments prepared `2026-08-31` | Exact eight-object implementation-blind author-packet candidate; both future contract views must develop shared-ID options for weighting/noise authority, quantitative approximation conformance, covariance representation, immutable-state persistence, response/product/FLT→FRUIT persistence, and VAL profile granularity | Scientific owner must approve the exact packet bytes/hashes and explicitly authorize Stage B; authored options later require separate owner disposition before freeze or numerical authorization | Review the exact manifest-bound packet; do not launch Stage B, choose an option, or claim algorithm/conformity/validation yet |
