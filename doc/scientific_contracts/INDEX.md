...
| [SCI-FLT-INF — inference-bearing filtering holding study](studies/SCI-FLT-INF_STAGE_A_2026-08-30/README.md) | Recovery-first Stage A study complete for owner walkthrough; explicitly not a package or combined contract; ODQ-001 and ODQ-002 approved and closed `2026-08-30` | Historical full path is an optimal matched-template amplitude estimator with amplitude-unbiased normalization under declared assumptions; selected package owns a map-domain operation whose signal output is a matched-filtered map preserving applicable parent map structure/semantics; source analysis is excluded with no SRC ownership boundary; posterior/Wiener reconstruction and ordinary convolution remain distinct; exact frozen-NOI binding; protected SCI-FLT-FIXED bytes unchanged | `SCI-FLT-INF-ODQ-003`: exact admitted input map parent or parents and observation/coadd grouping; later operator/state/product decisions depend on it | Resolve ODQ-003 next; no package-local Stage A author packet, Stage B launch, or algorithm/conformity/validation claim yet |
