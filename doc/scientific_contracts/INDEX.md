...
| [SCI-MAP — ordinary mapmaking and observation coaddition](packages/SCI-MAP/v0.1/README.md) | Owner-approved v0.1 scope; final targeted closure packet rendered `2026-08-28` | Formal, rationale, and ECS synchronized at r0.7; byte-identical PTC/MAP boundary, source-current atomic and aggregate VAL profiles, ordered typed-gate DAG, original-footprint coordinate boundary, and three separated response families; 52 requirement and 25 prediction IDs retained; authority not frozen | Hard numerical gates: exact PTC MAP-facing coefficient family and admitted `coverage_cut` numerical domain. Eight MAP owner decisions remain open; OD-008 is resolved | Scientific-owner content review of r0.7; no conformity, validation, achieved response/performance, freeze, readiness, or production claim |
...
| [SCI-VAL — sample/detector validity, flags, and map eligibility](packages/SCI-VAL/v0.1/README.md) | Owner-approved r0.3 Core; continuing Registry preserves MAP `@1` history and registers source-current `SCI-MAP:map_upstream_admission@2` plus `SCI-MAP:observation_coadd_admission@1` | Core remains r0.3 with 49 requirements and 24 predictions; continuing Registry/source binding updated `2026-08-28`; both policies are MAP-authored | No general VAL science question open; missing PTC coefficient makes the occurrence profile unavailable at that required gate | Preserve VAL as evaluator/registry; no conformity, validation, or readiness claim |
