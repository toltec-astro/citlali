...
| [SCI-FRUIT — iterative feedback, state, and termination](packages/SCI-FRUIT/v0.1/README.md) | Recovery-first Stage A owner-review candidate launched `2026-08-31` from exact FLT-FIXED freeze commit `7f9307ff...`; implementation evidence quarantined; candidate scope not approved | Four separate ordinary MAP, JINC, FLT-FIXED, and provisional FLT-MATCHED routes classified; iterative DAG, state/restart taxonomy, claim matrix, and owner-decision queue drafted; no route numerically admitted and no Stage B launch | ODQ-001 first: select the feedback-model estimand and recurrence/update law; all later route, projector, state, response, stopping, restart, uncertainty, and product questions remain open | Conduct scientific-owner walkthrough one question at a time; do not dispatch an author packet or alter algorithms/frozen authorities |
