...
| [SCI-JINC — signed-coefficient JINC gridding and response](packages/SCI-JINC/v0.1/README.md) | Approved predecessor Stage A packet at `6639bff3d94b92ace8faf3e407ccaefd5a38ea1f`; bounded ODQ-101 successor candidate awaiting renewed exact-byte approval | ODQ-101 now assigns one PTC-owned positive coefficient-family registry with explicit MAP/JINC permissions and staged selection; JINC retains signed `kappa`/`w` and all JINC semantics; no family is registered here; frozen MAP/PTC unchanged; Stage B not commissioned | Next: `SCI-JINC-ODQ-102B`, exact TolTEC per-array scales and parameter source/value disposition; then VAL successor, numerical/phase/cache policy, outside-center edge rule, residual gaps and `SCI-JINC-STAGE-A-Q002` | Review the exact ODQ-101 successor bytes; do not commission Stage B before renewed approval and remaining dispositions |
