...
| [SCI-FLT-INF — inference-bearing filtering holding study](studies/SCI-FLT-INF_STAGE_A_2026-08-30/README.md) | Recovery-first Stage A study complete for owner walkthrough; explicitly not a package or combined contract; ODQ-001 through ODQ-003 approved and closed by `2026-08-31` | Historical full path is an optimal matched-template amplitude estimator with amplitude-unbiased normalization; selected package publishes matched-filtered maps; both ordinary-MAP observation and coadd bundles are admitted as distinct observation-local/coadd-local parents with no equivalence or filter-owned combination; JINC/derived parents and source analysis are excluded; posterior/Wiener reconstruction remains separate; exact frozen-NOI binding; protected SCI-FLT-FIXED bytes unchanged | `SCI-FLT-INF-ODQ-004`: exact noise/covariance model, weighting, and parent-coefficient authority for each admitted parent; later operator/state/product decisions depend on it | Resolve ODQ-004 next; no package-local Stage A author packet, Stage B launch, or algorithm/conformity/validation claim yet |
