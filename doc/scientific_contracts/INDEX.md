...
| [SCI-NOI — realization ensembles, empirical uncertainty, and standardized signal](packages/SCI-NOI/v0.1/README.md) | Final Stage A owner-review closure candidate under one-at-a-time review on `2026-08-29`; ODQ-101/102A/102B approved | Fixed-state conditional-sign ordinary family; selected NOI-owned PTC-to-frozen-MAP route with optional inline MAP application and NOI realization-map output; one assignment per observation-scoped detector applies to all its admitted samples; numerical assignment remains unavailable pending design and coefficient/`coverage_cut` gates; Stage B not launched | `SCI-NOI-ODQ-102C` next: exact sign law and finite design; later decisions and exact final packet/profile/source approval remain open | Walk decisions one at a time from ODQ-102C, then complete exact profile/Registry/source bindings before conditional implementation-blind Stage B dispatch |
