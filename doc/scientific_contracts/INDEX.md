...
coordinate/projection boundaries. The later MAP r0.5 targeted closure selects
the initial one-hot projection while preserving its exact source authority.
...
| [SCI-MAP — ordinary mapmaking and observation coaddition](packages/SCI-MAP/v0.1/README.md) | Owner-approved v0.1 scope; targeted PTC-to-MAP cross-package closure rendered `2026-08-27` | Formal, rationale, and ECS synchronized at r0.5; exact PTC-to-MAP boundary, registered MAP admission profile, one-hot projection, exposure carriage, and base coadd profiles; 52 requirement and 25 prediction IDs retained; authority not frozen | Hard numerical gates: exact PTC MAP-facing coefficient family and admitted `coverage_cut` numerical domain. Eight MAP owner decisions remain open; OD-008 is resolved | Scientific-owner content review of r0.5; no conformity, validation, achieved response/performance, freeze, readiness, or production claim |
...
| [SCI-VAL — sample/detector validity, flags, and map eligibility](packages/SCI-VAL/v0.1/README.md) | Owner-approved r0.3 Core; continuing Registry binds actual policy ownership and now registers exact `SCI-MAP:map_upstream_admission@1` | Core remains r0.3 with 49 requirements and 24 predictions; continuing Registry/source binding updated `2026-08-27`; MAP policy is MAP-authored and occurrence-level | No general VAL science question open; missing PTC coefficient makes the MAP profile decision unavailable for that required gate | Preserve VAL as evaluator/registry; no conformity, validation, or readiness claim |
