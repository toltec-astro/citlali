...
| [SCI-FLT — map-domain filtering tranche](packages/SCI-FLT/v0.1/README.md) | Final repaired SCI-FLT-FIXED Stage A candidate complete on `2026-08-30`; `SCI-FLT-INF` retained only as a holding tranche; internal dossier quarantined | Strict-linear same-grid `y=J_full L_Theta m`; fixed convolution plus qualified low-pass subtype; full-footprint-only sole edge method; exact MAP/JINC/NOI boundaries, decision tables, VAL drafts, atomic lifecycle, resolved owner record, and SHA-bound 17-object candidate packet; Stage B not authorized | No bounded scope decision remains open; exact repaired bytes/hashes require owner approval | Obtain exact-byte approval and explicit launch before one fresh SCI-FLT-FIXED Stage B author; no SCI-FLT-INF, implementation, conformity, validation, calibration, performance, readiness, production, or freeze action |
| [SCI-FLT-INF — inference-bearing filtering holding study](studies/SCI-FLT-INF_STAGE_A_2026-08-30/README.md) | Recovery-first Stage A study complete for owner walkthrough; explicitly not a package or combined contract; ODQ-001 through ODQ-003 approved and closed and ODQ-004 author-delegated by `2026-08-31` | Historical full path is an optimal matched-template amplitude estimator with amplitude-unbiased normalization; selected package publishes matched-filtered maps; both ordinary-MAP observation and coadd bundles are admitted as distinct observation-local/coadd-local parents with no equivalence or filter-owned combination; JINC/derived parents and source analysis are excluded; posterior/Wiener reconstruction remains separate; ODQ-004 requires bounded noise/covariance, spectral-weighting, and parent-coefficient options in both future contract views, with the historical radially symmetrized average map noise PSD only a candidate; exact frozen-NOI binding; protected SCI-FLT-FIXED bytes unchanged | `SCI-FLT-INF-ODQ-005`: exact template/kernel identity; the owner must separately select or dispose of the future authored ODQ-004 options before freeze or a numerical route | Resolve ODQ-005 next and preserve the ODQ-004 two-view author assignment; no package-local Stage A author packet, Stage B launch, or algorithm/conformity/validation claim yet |
