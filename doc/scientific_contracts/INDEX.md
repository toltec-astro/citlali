...
| [SCI-FLT-INF — inference-bearing filtering holding study](studies/SCI-FLT-INF_STAGE_A_2026-08-30/README.md) | Recovery-first Stage A study complete for owner walkthrough; explicitly not a package or combined contract | Separates template-amplitude, posterior/Wiener, source/catalog amplitude, state-learning lifecycle, mode-selection, selector/fallback, adaptive-edge, and NOI-derived coefficient families; exact frozen-NOI binding; protected SCI-FLT-FIXED bytes unchanged | `SCI-FLT-INF-ODQ-001`: intended estimand of the existing full path; later package split and operator/state/product decisions depend on it | Resolve ODQ-001 first; then create separate package-local Stage A packets only for owner-selected estimands; no Stage B launch or algorithm/conformity/validation claim |
