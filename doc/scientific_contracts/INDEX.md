...
| [SCI-NOI — realization ensembles, empirical uncertainty, and standardized signal](packages/SCI-NOI/v0.1/README.md) | Final Stage A owner-review closure candidate under one-at-a-time review on `2026-08-29`; ODQ-101 and ODQ-102A approved | Fixed-state conditional-sign ordinary conditioning family; selected NOI-owned PTC-to-frozen-MAP route with optional inline MAP application and NOI realization-map output; selected route remains numerically unavailable at coefficient/`coverage_cut` gates; other routes unselected; complete design/source/UNC/STD/lifecycle tables; GEN-owned completion; SHA-bound packet; Stage B not launched | `SCI-NOI-ODQ-102B` next: initial coherence-unit family; later decisions and exact final packet/profile/source approval remain open | Walk decisions one at a time from ODQ-102B, then complete exact profile/Registry/source bindings before conditional implementation-blind Stage B dispatch |
