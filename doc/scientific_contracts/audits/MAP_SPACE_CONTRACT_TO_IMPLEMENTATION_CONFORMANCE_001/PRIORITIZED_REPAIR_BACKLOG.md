...
| P0 | Implement frozen ordinary MAP observation bundle (`CTI-RU-MAP-OBS-001`) | MSP-P004, MSP-E001--MSP-E003, MSP-T001; parent prerequisite for MSP-E004/MSP-E009/MSP-E012/MSP-E023 | Bind exact immutable identity, PTC/VAL admission, coefficient family, response/covariance states, support/validity, and unique-original exposure as specified below | Close affected authority gates first; SCI-MAP r0.7.1 mapping review, identity/admission fixtures, typed-unavailability and atomic rejection checks |
...
## Ordinary MAP observation-bundle unit

`CTI-RU-MAP-OBS-001` is limited to the MSP-P004 ordinary observation bundle
under the frozen SCI-MAP v0.1/r0.7.1 authority already bound by
`SOURCE_AUTHORITY_MANIFEST.md`.  Its required outcome includes:

- **Identity:** one immutable observation/array bundle with exact parent and
  lifecycle generation, ordinary estimator/quantity, unit and calibration
  lineage, full-precision WCS/shape, and required companion identities.
- **Admission and coefficients:** bind the exact PTC-to-MAP boundary and
  atomic/aggregate VAL decisions to the applicable occurrence, selected
  PTC-owned coefficient family, unit, QC/profile and missing/nonfinite rules.
  Retain the separate normalization-support, science-policy-support and
  raw-validity states, including exact owner-admitted numerical
  `coverage_cut` and support-policy population.  Legacy weights, empirical
  NOI scales, and the coadd-only `u_op=1` rule supply no missing sample
  coefficient authority.
- **Response and covariance:** retain exact compatible companion identities
  and availability under SCI-MAP-REQ-008/016/019/023, keeping fixed-state,
  PTC full-procedure and PTC+MAP re-resolved response distinct and covariance
  conditional on its declared state/domain.  Unavailable state cannot become
  zero, unity, or independence.  Missing response does not invalidate an
  otherwise valid base map; stronger covariance claims require their own
  authority.
- **Unique-original exposure:** consume the separately repaired original-
  exposure path with its exact occurrence/ALIGN mapping identity; deduplicate
  and place each original at its own AST ALIGN-grid coordinate in the target
  MAP WCS.  Preserve both upstream-eligible and retained original-footprint
  products independently of processed signal membership and support.
- **Atomic closure:** reject missing or mismatched required identity,
  authority, admission, or companion state before publication or mutation;
  preserve explicitly permitted unavailable states.  Check positive and
  negative identity/admission cases and original deduplication before any
  downstream unit consumes the bundle.

The PTC coefficient-authority and original-exposure units are prerequisites,
not substitutes for this bundle unit.  Any still-open numerical authority
must receive separate owner disposition before affected implementation.
Coaddition, JINC, filtering, NOI method changes, POINT, FRUIT, OOF, Registry
authoring, and application validation remain outside this unit.
...
exposure; ordinary MAP observation bundle (`CTI-RU-MAP-OBS-001`);
equal-observation coadd; five-role JINC;
