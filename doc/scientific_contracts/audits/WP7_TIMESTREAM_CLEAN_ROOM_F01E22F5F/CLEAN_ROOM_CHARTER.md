...
Audit the separately requested RTC-only terminal route that ends after RTC and
may supply conditioned timestreams to an external consumer. Do not infer a
CAL, PTC, or mapping product on that route.
...
listed in the source manifest and readable-source allowlist.
