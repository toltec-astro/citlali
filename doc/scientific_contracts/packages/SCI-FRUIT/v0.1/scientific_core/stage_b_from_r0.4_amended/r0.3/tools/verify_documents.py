...
def main():
    ...
    cross=(ROOT/'R0.2_TO_R0.3_CROSSWALK.md').read_text()
    for prefix,ids in [('REQ',req),('ASM',asm),('PRED',pred)]:
        for i in ids:assert cross.count('SCI-FRUIT-'+prefix+'-'+i)==1,(prefix,i)
    for p in (ROOT/'src/common').glob('*.tex'):assert sha(p) in cross and sha(hist/'src/common'/p.name) in cross
    b=json.loads((ROOT/'reports/BUILD_RECORD.json').read_text());assert b['identity']=='SCI-FRUIT-DOC-BUILD-R0.3-2026-09-08' and b['clean_build']['all_pdf_bytes_identical']
    ...
    for view,expected,title in [('core',28,'SCI-FRUIT-NORMATIVE-CORE'),('rationale',10,'SCI-FRUIT-SCIENTIFIC-RATIONALE'),('ecs',13,'SCI-FRUIT-ENGINEERING-CONFORMANCE')]:
        ...
        meta=dict(reader.metadata);assert meta['/Title']==title+' v0.1/r0.3' and meta['/Author']=='Grant Wilson'
    ...
    outer={ROOT/'SCI-FRUIT-v0.1-stage-b-r0.3-owner-review.tar.gz',ROOT/'SCI-FRUIT-v0.1-stage-b-r0.3-owner-review.tar.gz.sha256'}
