"""Package exact r0.3 documents; outer archive/digest are external sibling files."""
from pathlib import Path, PurePosixPath
import argparse,gzip,hashlib,io,json,re,subprocess,sys,tarfile
ROOT=Path(__file__).resolve().parents[1]
NAME='SCI-FRUIT-v0.1-stage-b-r0.3-owner-review.tar.gz'
ARCHIVE=ROOT/NAME
...
def role(p):
    ...
    if rel.startswith('pdf/'):return 'Completed r0.3 review-candidate PDF'
...
def verify(archive):
    ...
    expected={'r0.3/'+p.relative_to(ROOT).as_posix():p for p in files+[MANIFEST,SIDE]}
    with tarfile.open(archive,'r:gz') as tf:
        members=tf.getmembers();assert len(members)==len(expected) and {m.name for m in members}==set(expected)
        for m in members:
            path=PurePosixPath(m.name)
            assert not path.is_absolute() and '..' not in path.parts and '.' not in path.parts and path.parts[0]=='r0.3',m.name
...
def main():
    ...
    files=payloads()
    lines=['# Exact delivery manifest — SCI-FRUIT Stage B r0.3','','Status: complete review-candidate delivery; owner approval, freeze and activation pending. Numerical methods/routes remain unavailable_pending_separate_owner_approval; comparisons blocked; evidence artifacts not produced.','',f'Complete sole current Stage B review-candidate normative core: SCI-FRUIT-NORMATIVE-CORE v0.1/r0.3. Canonical source-inventory SHA-256: `{checks["core_source_inventory_sha256"]}`.','','Every regular payload appears once. ARTIFACT_MANIFEST.sha256 binds this manifest. The archive contains these payloads plus manifest/sidecar. The outer archive and archive-digest sidecar are external sibling artifacts, excluded from members and in-tree links. Task caches, build intermediates, rendered PNGs and Python caches are excluded. Historical input archives remain opaque bytes. Every member is a regular file with a safe relative path; no symlink or hard link is admitted.','','| Path | Role | Bytes | SHA-256 |','| --- | --- | ---: | --- |']
        ...
        with gzip.GzipFile(fileobj=raw,mode='wb',filename='',mtime=EPOCH) as gz:
            with tarfile.open(fileobj=gz,mode='w',format=tarfile.PAX_FORMAT) as tf:
                for p in sorted(files+[MANIFEST,SIDE]):
                    content=p.read_bytes();info=tarfile.TarInfo('r0.3/'+p.relative_to(ROOT).as_posix());info.size=len(content);info.mode=0o644;info.mtime=EPOCH;tf.addfile(info,io.BytesIO(content))
