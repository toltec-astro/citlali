# Document build recipe - SCI-FRUIT v0.1/r0.2

Build-record identity: SCI-FRUIT-DOC-BUILD-R0.2-2026-09-07. Scope: local document compilation, rendering and identity checks only.

Run from any directory using the script's absolute path, or from the extracted `r0.2` directory:
...
The [packaging tool](tools/package_documents.py) writes the complete final manifest and archive only after document checks pass. It includes every regular delivery payload plus manifest/sidecar, excludes task caches/intermediates/renders and archive self-reference, and checks exact archive-member bytes. Historical Stage A/r0.1 and pre-interim r0.2 candidate archives are copied as opaque payloads and never unpacked by these tools. The separately supplied in-progress snapshot outside r0.2 is not a member.
