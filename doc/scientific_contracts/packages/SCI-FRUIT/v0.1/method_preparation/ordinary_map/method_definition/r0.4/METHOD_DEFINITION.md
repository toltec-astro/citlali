...
Paper revision r0.4, 2026-09-09. Scientific definition accepted and frozen under
[the scoped owner decision](SCIENTIFIC_OWNER_FREEZE_R0.4.md).
Retained specialization identity:
`FRUIT-FEEDBACK-METHOD/ordinary-map-mode-aware-relearning@r0.3`.
The revision clarifies scope, without changing the reference algorithm or
creating a new numerical method. Q01 acceptance covers this scientific
definition; numeric policy instantiation and Q02's exact upstream amendments
remain separate. Every B01–B12 reference below is conditional on separate
adoption, including the coefficient/QC and correction-embedding proposals.
This specializes the frozen generic core; it neither edits it nor selects a
universally best FRUIT policy. Full numerical-method authority remains
...
These fixed-domain, contributor, PTC-support and association restrictions
belong to this controlled ordinary-MAP reference, not to FRUIT generally.
Another explicitly declared method may permit validity/support changes caused
by operations in the chain. It must retain before/after support identities,
the responsible operation, reason and affected products/claims, and a declared
comparison domain that exposes any loss. Processing invalidity or support loss
must not be relabeled as inference rejection or evidence of convergence.
This clarification does not relax the fixed-support failure rules of this
reference or activate a support-changing successor.
...
Binary admission multiplied by unfiltered reconstructed-total flux is the
leading ordinary-MAP reference estimator, not a universal FRUIT requirement.
The general inference contract separates admission, flux estimation and
application, without requiring every future estimator to emit a binary mask.
A separately declared fitted astronomical component, including a continuous
spatial model, can be legitimate under its own estimator, response, uncertainty
and approval conditions. It is not the same operation as multiplying measured
flux by confidence. No such alternative is selected by this clarification.
...
| --- | --- |
| 01 Authority/lineage | Q01 scientific definition accepted/frozen; numerical policy instantiation, Q02 amendments, configuration and execution remain separate. |
