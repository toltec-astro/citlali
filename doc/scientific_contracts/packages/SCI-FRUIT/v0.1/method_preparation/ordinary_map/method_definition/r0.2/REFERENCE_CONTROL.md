...
| `inputs/manager/` | false | Additional frozen PTC/CAL boundary recovery, exact MAP source-lock copies, prior internal dossier/scope/decision material and the supplied relearning discussion. Manager review only; not automatically admitted to future independent authorship. |
...
This retains the three corrected process-reference flags from method-definition
r0.1 without changing the 24-entry proposed-reference inventory. The reviewed
ordinary-MAP scope r0.2 packet and tar.gz retain their original bytes,
...
The r0.2 successor adds only the exact supplied relearning discussion to the
manager inputs. The [separate owner-direction record](RELEARNING_DIRECTION_AND_REVIEW.md)
captures conversational assent and its scope. The discussion and its linked
papers are review context; none changes the proposed author-reference set or
imports a candidate upstream contract. The original scope approval is copied
byte-for-byte and remains distinct from this later learning-direction approval.
