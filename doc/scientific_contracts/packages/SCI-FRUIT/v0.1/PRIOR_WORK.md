...
- Governing repository conventions: `doc/SCIENTIFIC_CONVENTIONS.md`,
  `doc/ARCHITECTURE.md`, and `doc/adr/0006-fruit-loop-restart-checkpoint.md`.
...
| `FRUIT_LOOP_FEEDBACK_INVESTIGATION_2026-07-24.md` | historical implementation/validation evidence | Records the subtract-residual-clean-restore-complete-map recurrence; controlled ablations separated projection, PTC recovery, support, and restart effects; v2 restart equality observed | Current algorithm behavior is not science authority; v1 trajectory was invalid because state was incomplete; no additive reformulation proof | **abstract** exact compatibility baseline/questions; **exclude** from authorship |
...
| Historical implementation at `f70701ad...` and byte-identical core recurrence files at `7f9307ff...` | implementation evidence | Each iteration rereads original observations; carries one selected complete route map bundle; constructs/selects/projects a model; runs residual-only PTC/weights/noise; restores the model; creates the next complete route product; carries separate learned/checkpoint state | Behavior may be incomplete or scientifically wrong and cannot enter author packet; not an additive-equivalence proof | **recover** owner-facing compatibility baseline in quarantined Stage A; **exclude** implementation from authorship |
| Focused recurrence and restart tests at `f70701ad...`/`7f9307ff...` | test evidence | Immediate sample-domain round trip, controlled fixed-linear recurrence, effective learning-state and weight-validation checkpoint round trips | Do not prove production nonlinear/additive equivalence, response/calibration, or universal checkpoint completeness | **cite** evidence limits internally; **exclude** from authorship |
...
- Historical code carries one selected complete route map bundle, constructs a
  selection-supported projected model from it, subtracts/restores that model,
  and replaces the numerical predecessor with a newly completed route product.
  The scientific boundary still requires a typed accepted `F_k`; the owner must
  decide whether to preserve this law, prove an equivalent reformulation, or
  intentionally adopt a new recurrence.
- Explicit additive residual accumulation is not recovered behavior. It is
  equivalent only if projection/remapping, response, support, grid, weighting,
  normalization, masks, filtering, and learning conditions are proved; those
  proofs are unavailable.
- Update-contribution scientific status and persistence are independent of the
  state identity and transition law. No recovered authority calibrates an
  increment or requires unbounded retention.
...
1. the carried accepted feedback-state identity;
2. preservation, proven equivalence, or intentional replacement of the
   historical numerical transition;
3. update-contribution scientific/diagnostic status and bounded persistence;
4. admitted parent route or routes and the model-construction operator for each;
5. forward projection, removal/residual-only/bypass/rejoin semantics, response,
   and null space;
6. fixed versus learned/relearned state and its generation graph;
7. support, selection, validity, failure, and terminal product bundle;
8. diagnostic convergence versus stopping and terminal-selection rules;
9. uncertainty/covariance scope, including fixed-state and replay variants; and
10. exact checkpoint completeness and compatibility rules implied by those
   choices.
