...
\newcommand{\DocPdfTitle}{SCI-CAL v0.3 TolTEC Timestream Calibration Draft}
\input{common/preamble.tex}
\fancyhead[L]{\small\color{calgray}SCI-CAL v0.3 DRAFT}
\hypersetup{pdftitle={SCI-CAL v0.3 TolTEC Timestream Calibration Draft},pdfauthor={SCI-CAL scientific owner review revision},pdfsubject={Scientist-facing calibration rationale}}
...
\newcommand{\flowboxsmall}[1]{\fcolorbox{calteal}{callight}{\parbox[c][0.72in][c]{0.165\linewidth}{\centering\footnotesize #1}}}
...
{\large Scientist-facing rationale, revision 0.3 - DRAFT}\\[0.10in]
...
{\normalsize Surgical correction prepared from the owner assessment of v0.2}
\end{titlepage}
...
calibration factor from the selected observation-specific APT, whose
calibration descends from Beammap calibration evidence, and one atmospheric
correction for the target sample. For detector (d), time
...
\textbf{Numerical atmosphere status} & The materials available for this draft do not contain the operator nodes, ordinate orientation, or numeric support. Numerical atmosphere evaluation and calibrated numerical output are not supported by this draft.\\
...
selected observation-specific APT records the target-to-source detector
association. Each supported target detector has one corresponding
source-calibration row; an ambiguous or unsupported match produces no
calibrated value for that detector.
...
The materials available for this draft call \code{xs} the ordinary science
stream but do
...
\DecisionBox{Input signal and baseline}{The relevant scientific owner must
define and approve a scientific
definition of ordinary \code{xs}: physical observable, unit, sign, prior
normalization and additive processing, stream scope, and the loading,
...
Here \(R\) is not the APT field \code{responsivity}. This
motivates factor direction; it does not establish the missing physical
definition of \code{xs} or the derivation of \(R\).
...
\flowboxsmall{TolProj-selected\\observation-specific child APT}\\[0.08in]
\makebox[\linewidth][r]{\(\downarrow\)\hspace{0.22\linewidth}}\\[-0.02in]
...
\end{tabular}
\caption{Scientific lineage from calibrator evidence to target-observation calibration. Beammap calibration produces the source APT factor; TolProj selects or creates the observation-specific child APT; SCI-CAL consumes its selected \code{flxscale} and applies the target atmosphere once.}\label{fig:lineage}
\end{figure}

SCI-CAL lies between upstream definition of the signal, APT, opacity, and
airmass and downstream filtering, weighting, mapmaking, and inference. The
...
\label{eq:noncommute}
\end{align}
Common-mode removal and PCA mix detectors; a PCA basis may also be
data-dependent. The materials available for this draft do not state the
complete governed ordering relative to these operations, temporal filtering,
or baseline estimation. They do
...
only explicitly approved child transformations. SCI-CAL consumes the selected
child row as the value boundary, applies its \code{flxscale} once, and applies
target atmosphere. The downstream MAP/FLT response owner owns the realized
mapmaker, gridding, kernel, and filter response.

This follows the producer--transformer--consumer rule: the producer owns a
quantity's meaning, a transformer owns only its approved transformation and
lineage, and a consumer applies the selected value without reinterpretation.
If
...
\DecisionBox{Calibration derivation and transfer}{The materials available for
this draft do
...
calibrator to target. The relevant scientific owner must define and approve
the generating contract and decide
...
\begin{equation}
 ...
 H_a(t)=\mathcal O_a[\ell_o(t)],\qquad
 (T_a,C_a)=
 \begin{cases}
 (H_a,H_a^{-1}),&H_a\text{ is transmission},\\
 (H_a^{-1},H_a),&H_a\text{ is correction}.
 \end{cases}
...
\DecisionBox{Authoritative atmosphere operator}{The materials available for
this draft
do not contain the per-array nodes and ordinates, ordinate orientation,
...
absent from the materials reviewed. The relevant scientific owner must
classify the status and define and approve one
complete immutable record. Until then correction curves, numerical examples,
an approximation-error plot, numerical atmosphere evaluation, and calibrated
numerical output are not supported. Supplying the record enables
structural evaluation; it does not validate atmospheric fidelity.}

The calibration path defined by the present contract applies only for
...
the materials reviewed and remain owner decisions.
...
\DecisionBox{Broadband photometric convention}{The relevant scientific owner
must define and approve the
...
\begin{table}[ht]\centering\small
\caption{Science-facing uncertainty budget. ``Not established'' means the materials available for this draft do not state a present numerical product.}\label{tab:uncertainty-budget}
...
\caption{Decision register for the v0.3 science rationale.}\label{tab:decisions}\\
...
Q06 & Atmosphere operator & Nodes, ordinates, orientation, support, weighting, model, content identity; classify project-wide versus reviewed-material absence & Blocks calibrated numbers \\
...
assumptions 001--011, and edge predictions 001--030. This v0.3 changes genre
and presentation, not their algebra.

\section{Crosswalk to the engineering contract}\label{app:crosswalk}
\input{scientific-crosswalk-v0.3.tex}
\end{document}
