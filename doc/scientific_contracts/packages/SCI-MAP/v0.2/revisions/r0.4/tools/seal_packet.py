...
def seal(root):
    ...
    manifest={
        'identity':'SCI-MAP-v0.2-r0.4-FINAL-DOCUMENT-SOURCE-MANIFEST/2026-09-08',
...
def archive(root,path):
    ...
    with path.open('xb') as raw, gzip.GzipFile(filename='',mode='wb',fileobj=raw,mtime=1788825600) as zipped, tarfile.open(fileobj=zipped,mode='w',format=tarfile.PAX_FORMAT) as tar:
        for p in sorted(root.rglob('*')):
            ...
            blob=p.read_bytes(); name='SCI-MAP-v0.2-r0.4/'+p.relative_to(root).as_posix()
    ...
    with tarfile.open(path,'r:gz') as tar:
        ...
        for member in tar.getmembers():
            assert member.isfile() and member.name.startswith('SCI-MAP-v0.2-r0.4/')
            rel=member.name.removeprefix('SCI-MAP-v0.2-r0.4/')
