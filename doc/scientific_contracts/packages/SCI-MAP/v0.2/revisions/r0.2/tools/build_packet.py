...
CORE = ['SCI-MAP-v0.2_SHARED_AUTHORITY_r0.2.tex'] + [f'common/{x}.tex' for x in ('notation','definitions','equations','assumptions','requirements','edge_cases')]
CORE_ID = 'SCI-MAP-v0.2-SHARED-AUTHORITY/r0.2'
MANIFEST = 'doc/scientific_contracts/studies/SCI_MAP_POST_FREEZE_RECONCILIATION_2026-09-07/AUTHOR_REFERENCES.json'
DIRECTIVE = 'owner/SCI_MAP_R02_OWNER_DIRECTIVE.txt'
...
def prepare(args):
    ...
    build_id = 'SCI-MAP-R02-DOCUMENT-BUILD/' + attempt.name
    data = {
        ...
        'compile_environment': {'SOURCE_DATE_EPOCH': '1788739200', 'TZ':'UTC'},
    ...
    for view, (entry, _) in VIEWS.items():
        ...
        rows = [
            ...
            ('Contract / document', 'SCI-MAP v0.2 / r0.2'),
            ('Status / date', 'Candidate for scientific-owner disposition / 2026-09-07'),
...
def inspect(args):
    ...
    for view,(entry,name) in VIEWS.items():
        ...
        bad=[line for line in logtext.splitlines() if re.search(r'Overfull|undefined references|Reference .+ undefined|Citation .+ undefined|Missing character|^!',line,re.I)]
