# SCI-MAP v0.2/r0.2 full-procedure response comparison-space specification
...
`Delta z_PTC-FP`, `Delta m_PTC-FP`, and `Delta m_PTC+MAP-RR`. Baseline and
perturbed arrays are never subtractable merely because both exist or have the
same shape.
...
| --- | --- |
| `comparison_record_id` | Immutable record identity and generation. |
| `response_family` | Exactly one of PTC full-procedure, fixed-MAP projection of PTC full-procedure, or PTC+MAP re-resolved full procedure. |
