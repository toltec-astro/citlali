# SCI-MAP v0.2/r0.2 unified lifecycle and parameter-state table
...
| 5 | `realized` | Immutable resulting accumulators, products, statuses, failures, and provenance were actually produced. | Create only for actual realization. It never authorizes the earlier application. |
...
| Outcome | Allowed MAP records |
| --- | --- |
| Completed realization | All five states, in order, with exact values and identities. |
| Stop before application | Only the reached prefix through at most `observation_resolved`; no `applied` or `realized` record and no future-stage placeholder. |
| Applied attempt that fails before output realization | Reached prefix through `applied`, exact attempt identity, terminal failure, cause, and scope; no `realized` product or completion marker. |
