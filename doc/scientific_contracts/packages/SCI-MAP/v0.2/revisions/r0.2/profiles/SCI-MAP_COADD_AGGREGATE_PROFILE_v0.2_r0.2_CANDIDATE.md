# SCI-MAP coadd aggregate profile v0.2/r0.2 candidate

Profile identity: `SCI-MAP:observation_coadd_admission@2`.

Status: **CANDIDATE successor profile for scientific-owner disposition**. It
...
Policy owner: Grant Wilson for SCI-MAP. VAL registers and evaluates this
MAP-authored policy and performs no coadd arithmetic.

Source binding: paired candidate records
`SCI-VAL_PROFILE_REGISTRY v0.1/r0.3-map-v0.2-r0.2-candidate-2026-09-07` and
...
`SCI-MAP:map_upstream_admission@2` under the r0.2 candidate binding.
...
- `SCI-MAP:response_bearing_coadd@1` requires every response object named by
  that selected request with exact compatible source domain, basis, class,
  unit, normalization, parent, WCS/grid, and row identity. Absence of a
  different response family not named by the role is irrelevant.
