...
def seal(root):
    ...
    manifest={
        'identity':'SCI-MAP-v0.2-r0.3-FINAL-DOCUMENT-SOURCE-MANIFEST/2026-09-08',
        ...
        'files':files,
        'file_bytes':{name:(root/name).stat().st_size for name in files},
        ...
        'authority_limit':'Historical uniform/boundary approvals and literal identities are preserved. The generic scientific authority is a conditional-freeze candidate only. Affected draft-identity numerical routes and SCI-VAL evaluability/object decisions remain unavailable pending separately authorized immutable identities, binding and activation; no freeze or activation is performed.'
    }
...
def archive(root,path):
    ...
    with path.open('xb') as raw, gzip.GzipFile(filename='',mode='wb',fileobj=raw,mtime=1788825600) as zipped, tarfile.open(fileobj=zipped,mode='w',format=tarfile.PAX_FORMAT) as tar:
        for p in sorted(root.rglob('*')):
            if not p.is_file():continue
            blob=p.read_bytes(); name='SCI-MAP-v0.2-r0.3/'+p.relative_to(root).as_posix()
            info=tarfile.TarInfo(name);info.size=len(blob);info.mode=0o644;info.mtime=1788825600;info.uid=info.gid=0;info.uname=info.gname=''
    ...
    with tarfile.open(path,'r:gz') as tar:
        ...
        for member in tar.getmembers():
            assert member.isfile() and member.name.startswith('SCI-MAP-v0.2-r0.3/')
            rel=member.name.removeprefix('SCI-MAP-v0.2-r0.3/')
    ...
    return {'archive':str(path),'bytes':path.stat().st_size,'sha256':sha(path),'sidecar_bytes':path.with_name(path.name+'.sha256').stat().st_size,'sidecar_sha256':sha(path.with_name(path.name+'.sha256')),'verified_members':verified}
