#!/usr/bin/env python3
"""Read-only SCI-MAP r0.3 document-packet checks, not application evidence."""
import argparse
import hashlib
import json
...
def verify(root):
    ...
    check('136_exact_author_packet_files', len(inv['input_files'])==136 and all(sha(root/'inputs'/p)==d for p,d in inv['input_files'].items()))
    ...
    old_eq=(root/'inputs/basis-r0.2/src/common/equations.tex').read_text()
    ...
    old_owner=(root/'inputs/basis-r0.2/src/SCI-MAP-v0.2_OWNER_DECISION_REGISTER_r0.2.tex').read_text()
    new_owner=(root/'src/SCI-MAP-v0.2_OWNER_DECISION_REGISTER_r0.3.tex').read_text()
    check('nine_settled_rendered_owner_decisions_preserved',old_owner.replace('v0.2/r0.2','v0.2/r0.3',1)==new_owner)
    ...
    registry=root/'bindings/SCI-VAL_PROFILE_REGISTRY_SCI-MAP_v0.2_r0.3_CANDIDATE.md'
    register=root/'bindings/SCI-VAL_SOURCE_BINDING_REGISTER_SCI-MAP_v0.2_r0.3_CANDIDATE.md'
    ...
    for view,(entry,filename) in VIEWS.items():
        ...
        check(view+'_imports_shared_authority',r'\input{SCI-MAP-v0.2_SHARED_AUTHORITY_r0.3.tex}' in text)
        ...
        check(view+'_metadata_owner_revision_status', 'Grant Wilson' in meta.get('/Author','') and all(s in meta.get('/Title','') for s in ['v0.2','r0.3']) and 'candidate' in (meta.get('/Title','')+' '+meta.get('/Subject','')).lower() and meta.get('/CreationDate','').startswith('D:20260908'))
        ...
        check(view+'_cover_revision_owner', all(s in first for s in ['GrantWilson','v0.2','r0.3','2026-09-08']))
