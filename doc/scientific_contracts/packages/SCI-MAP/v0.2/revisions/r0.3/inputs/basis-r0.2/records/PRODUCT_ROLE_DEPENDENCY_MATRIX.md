# SCI-MAP v0.2/r0.2 product-role dependency matrix

Status: **CANDIDATE for scientific-owner disposition**. Exact role identities
are registered in
`../profiles/SCI-MAP_PRODUCT_ROLE_REGISTRY_v0.2_r0.2_CANDIDATE.md`. Normative
...
| `SCI-MAP:response_bearing_coadd@1` | Exact `SCI-MAP:base_coadd@1` and its identical ordered signal-member set | Every member response object named by this selected role with compatible source domain, basis, class, unit, normalization, parent, WCS/grid, and row identity | For an existing base parent, a required incompatibility blocks this role and never selects a subset. A separately requested admission may reject an observation before mutation only while constructing its own separately identified base/signal parent. No hidden subset or zero companion is permitted. |
...
`SCI-MAP:map_upstream_admission@2`, with the r0.2 candidate source binding.
