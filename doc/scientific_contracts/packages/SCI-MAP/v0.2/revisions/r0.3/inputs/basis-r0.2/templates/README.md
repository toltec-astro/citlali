# SCI-MAP v0.2/r0.2 machine-readable templates
...
| `MAP_LIFECYCLE_AND_PRODUCT_ROLE_RECORD.schema.json` | Selected product role, exact base parent/membership, reached MAP stages, and termination class. |
| `COVERAGE_CUT_OVERRIDE_RECORD.schema.json` | Typed `coverage_cut_above_one_override_requested` fact and its reached-stage bindings. |
| `FULL_PROCEDURE_COMPARISON_RECORD.schema.json` | Complete common comparison-space authorization for a numerical full-procedure response. |
| `WCS_COMPARISON_RECORD.schema.json` | Exact finite WCS population and representation comparison. |
| `VAL_HANDOFF_DECISION_RECORD.schema.json` | PTC/VAL request, applicability, eligibility, and decision-artifact realization axes. |
...
- `completed_realization` has all five stages; `pre_application_stop` has no
  `applied` or `realized` placeholder; and
  `post_application_pre_realization_failure` has an applied attempt and exact
  terminal cause but no realized product;
...
- evidence-artifact realization is assessed independently of the requirement
  result;
- a VAL Boolean projection is true only for
  requested/applicable/eligible/realized on the exact profile and object;
...
- a numerical procedure difference has every comparison-space field and two
