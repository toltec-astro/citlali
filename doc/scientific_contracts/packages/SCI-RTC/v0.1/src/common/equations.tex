% SCI-RTC v0.1/r0.6 shared normative core -- equations.
...
\begin{aligned}
 (\mathbf x^{A},\mathbf r^{A})
   &\xrightarrow{\ \text{pair admission, spike evidence, shift resolve}\ }
     \Omega_{xr},\\
 \mathbf x^{A}
   &\xrightarrow{\ A_\alpha,\,B^x_\omega,\,F_1\ldots F_K,\,D_{M,0}\ }
...
\begin{equation}
 ...
 \left(A_\alpha\mathbf x^{A}+\mathbf a^x_\alpha\right)
 +\mathbf b^x_{\Omega}\; .}
...
\begin{equation}
 \mathbf y^{x,\rm RTC}_{\mathcal R}=L^x_\Omega\mathbf x^{A}
  +\mathbf b^{x\prime}_\Omega,
 \qquad
 L^x_\Omega=D_{M,0}F_K\cdots F_1B^x_{\omega,\mathcal R}A_\alpha .
...
\begin{equation}
 ...
 =\mathcal T_{d,\zeta}\!\left(I^A_{dj},Q^A_{dj}\right),
 \qquad
 \mathcal J^{xr}_{dj}=(d,j,\text{stream},\zeta).
