% SCI-RTC v0.1/r0.4 shared normative core -- equations.
...
\begin{equation}
 \mathbf x^{\rm acq}
 \xrightarrow{\ A_\alpha,\,B_\omega,\,F_1\ldots F_K,\,D_{M,0}\ }
 \mathbf y^{\rm RTC}_{\mathcal R}
...
filtering, or phase-zero sampling.  The raw donor ratio in
equation~\eqref{eq:raw-donor} is convention transfer within $B_\omega$, not
...
\begin{equation}
 \boxed{\;
 \mathbf y^{\rm RTC}_{\mathcal R}=D_{M,0}
 F_{K,\theta_K}\cdots F_{1,\theta_1}
 B_{\omega,\mathcal R}
 \left(A_\alpha\mathbf x^{\rm acq}+\mathbf a_\alpha\right)
 +\mathbf b_{\Omega}\; .}
...
\begin{equation}
 \mathbf y^{\rm RTC}_{\mathcal R}=L_\Omega\mathbf x^{\rm acq}
  +\mathbf b'_\Omega,
 \qquad
 L_\Omega=D_{M,0}F_K\cdots F_1B_{\omega,\mathcal R}A_\alpha .
...
\end{align}
$D_{dn}$ identifies a directly selected synthesized or replaced occurrence.
...
$x^{(0)}$, initial fixed or identity plan $\Pi_0$, attempt index $a$, and
...
\end{equation}
Equation~\eqref{eq:iterative-refinement} separates finite attempts from the
accepted-plan sequence, defines the initial evaluation for nonidentity
