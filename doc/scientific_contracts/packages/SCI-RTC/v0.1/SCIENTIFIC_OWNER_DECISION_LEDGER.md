...
Status: Stage A scope and packet approved; eight downstream decisions remain
open
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `SCI-RTC-OD-001` | Primary `xs` signal producer with SCI-CAL/SCI-BEAM role owners | open | Physical observable, sign, reference/baseline, raw unit, calibrated-role unit, product-role admission matrix beyond the frozen Beammap boundary, and any donor transfer other than the approved valid-compatible-`flxscale` route | Universal interpretation of `xs`; use of a CAL operator in every non-BEAM role; scientifically interpretable cross-detector replacement outside the approved `flxscale_q / flxscale_d` case | Grant Wilson or named signal/CAL authority | — | Rationale; formal contract; adjacent CAL/BEAM interfaces |
