...
| Required facts | `S={s_bg}` and joint law; exact admitted membership; successful completion of every admitted member; `B_requested`, `B_resolved`, `B_completed`, `B_unique`, proposed `B_admitted_for_UNC`, design rank; complement/duplicate/dependence state; fixed/relearned graph; source-imprint state; for the ODQ-105B method, known zero center, exact normalized finite-design weights, no empirical recentering or `B-1`, and the exact common all-member finite-value domain; target, estimator, representation, response reference, effective information, estimator-uncertainty state, calibration, limitations, lifecycle, and provenance |
| Decisive exclusions | Any incomplete/failed/unavailable admitted member; failed GEN ensemble; survivor or partial-ensemble substitution; mixed fixed/relearned graph; mixed parent/method generation; method-invalid completed design; below the UNC method's positive minimum cardinality/rank; for ODQ-105B, empirical-mean subtraction, `B-1`, inferred equal weights, or member-subset use outside the common domain; incompatible unit/WCS/response/support; unavailable target/estimator identity; or missing source-imprint disclosure |
...
| Exact consumer action | Admit the exact complete all-members-successful GEN ensemble to exactly the named UNC estimator/domain; for ODQ-105B, authorize only the zero-centered design-weighted second moment on the common all-member domain. No UNC product exists until estimator realization and atomic publication succeed |
