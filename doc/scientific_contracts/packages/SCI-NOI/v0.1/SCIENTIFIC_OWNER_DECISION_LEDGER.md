...
| `SCI-NOI-ODQ-110C` | FRUIT fixed-state, successor, and replay scope | Decision artifact ODQ-110C | **decided — FRUIT retains source-model/recurrence/learning authority; exact fixed residual/terminal transform supports conditional uncertainty only under ODQ-110A parity; NOI-informed continuation creates a new immutable FRUIT/science/GEN/UNC generation; per-member replay is separate ODQ-104 method; no fixed/relearned mixing; all numerical routes remain gated** |
| `SCI-NOI-ODQ-111` | VAL profile identities and exact consumer actions | Decision artifact ODQ-111 | **decided — approve four NOI-owned `@1` profile identities and exact actions; producer facts remain producer-owned, NOI owns policy/action, SCI-VAL binds/evaluates only; no implicit action propagation; bytes remain unregistered/unevaluable pending exact Registry/source binding** |
