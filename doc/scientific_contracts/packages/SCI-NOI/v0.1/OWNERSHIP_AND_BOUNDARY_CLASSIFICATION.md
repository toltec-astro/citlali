...
| Randomization unit, sign/assignment law, balance, seed/key, membership | SCI-NOI Family G | Owned | These facts do not establish uncertainty |
| Realization availability, support, QC, persistence/reconstruction | SCI-NOI Family G | Owned | Availability is not sample validity or covariance adequacy |
| Target law, centering, finite correction, covariance domain, calibration | SCI-NOI Family U | Owned | “Empirical” does not mean physical or calibrated |
| Empirical variance/covariance | SCI-NOI Family U | Owned versioned companion | It does not become a formal MAP/JINC covariance retroactively |
| Marginal inverse variance, precision, consumer-effective weight | SCI-NOI Family U, each as a distinct role | Owned when separately authorized | They are not interchangeable and are not PTC/MAP coefficients |
| Standardized signal | SCI-NOI Family Z | Owned derived companion | It is not an uncertainty estimate, significance, or detection probability by itself |
...
1. A Family G output may exist with Family U unavailable.
2. A Family U output may be valid for one projection and unavailable for
   another.
3. A Family Z output requires a valid Family U scale but gains no stronger
