...
Artifact identity: `SCI-NOI_GEN_PARENT_OPERATOR_GRAPH v0.1/r0.1`

Status: ODQ-101 conditioning class approved; route-specific methods remain
owner-review candidates and numerically unavailable unless stated otherwise

## Conditioning Classes

For one fixed-state member `b`,

```text
realization_b = O_Theta0(R_b(parent)).
```

For one relearned member,
...
class. It does not select a numerical route. Relearned GEN remains unavailable
until an owner-approved graph names every replayed stage, input, learned state,
changed-state record, response, lifecycle, and failure.

## Distinct Route-Specific Method Candidates

| Candidate method identity | Earliest immutable parent and randomized quantity | Sign insertion and host operator | Output | Conservative state while route selection is open |
| --- | --- | --- | --- | --- |
| `NOI-GEN/PTC-TO-FROZEN-MAP-CONDITIONAL-SIGN@1` | Exact retained PTC occurrences carrying `Z_i^PTC` | Sign at the exact PTC occurrence/coherence unit, followed by one frozen MAP observation/coadd operator | Exact realization MAP bundle in inherited `mJy/beam` and WCS/support | Unavailable: no route selection, PTC MAP-facing coefficient, or owner-admitted numerical `coverage_cut` |
| `NOI-GEN/PTC-TO-FROZEN-JINC-CONDITIONAL-SIGN@1` | Exact retained JINC-admissible PTC occurrences carrying `Z_i^PTC` | Sign at the exact PTC occurrence/coherence unit, followed by one frozen JINC operator | Exact complete five-role JINC realization bundle when scientifically defined | Unavailable: no route selection; JINC coefficient, TolTEC parameter, and applicable numerical-adequacy authority remain unavailable |
...
## Required Identity For Every Complete Method

Every route-specific method binds:

1. exact method/version and conditioning class;
...
8. validity, response, covariance, availability, and omitted terms;
9. requested/effective/observation-resolved/applied/realized lifecycle;
