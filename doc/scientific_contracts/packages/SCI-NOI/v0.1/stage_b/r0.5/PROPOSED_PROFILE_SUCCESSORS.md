# SCI-NOI v0.1 r0.5 Proposed Profile Successors

Record identity: `SCI-NOI_PROPOSED_PROFILE_SUCCESSORS v0.1/r0.5`
...
| Scientific sources | r0.18 owner decisions; r0.3 owner supplement; r0.4 and r0.5 owner directives; byte-bound r0.5 normative modules. |
| Required facts | Explicit enabled request; positive integer `N_requested`; exact observation, array, occurrence, detector/channel, network, coefficient family/value/QC, stable identities/order, PTC validity/support/response, frozen MAP product/generation/projection/denominator/gates, numerical `coverage_cut`, and lossless canonical `a_pi` rational bound to exact `G_pi`, `gamma_i` family/generation/payload, representation source/digest, plus exact plan and generator/key identities, lifecycle and provenance. |
| Exclusions | Zero-mass occurrence from active signing; ambiguous positive-mass network identity; missing/conflicting/noncanonical fact; either zero-cardinality lifecycle state; cross-observation or cross-array ensemble; any changed parent/operator; rounded display decimal, independent quantization, re-estimation, another coefficient generation, undocumented precision/default, or floating admission decision. |
...
| Missing behavior | Missing profile authority or any required fact yields decision `unavailable`; explicit scientific nonmembership yields `ineligible`; `not_requested` and `explicitly_disabled` retain their distinct request/effective-plan states. No empty success. |
...
| Policy owner | Grant Wilson. |
| Scientific sources | r0.18 owner decisions; r0.3 owner supplement; r0.4 and r0.5 owner directives; byte-bound r0.5 normative modules. |
| Required facts | Exact GEN method/design/member identities; positive complete assignment resolution; member assignment and attempt identity; one-time sign application; exact parent/operator parity; support/WCS/response/unit; common-domain membership; duplicate/complement facts; persistence, lifecycle, cause, and provenance; and exact evidence that every binding completion/use predicate was fixed before assignment or is invariant under `epsilon_b` and `M_b`. |
| Exclusions | Rejected candidates; unresolved, unrealized, incomplete, failed, or wrong-method members; hidden survivor selection; output amplitude, sign pattern, source imprint, map morphology, tail value, or assignment-dependent QC as an exclusion; pairwise/missing-data subsets; changed operator/parent; missing authority/fact/invariance evidence. |
...
| Missing behavior | Missing authority/facts or assignment-dependent execution/numerical failure yields `unavailable` for the exact base method. A scientifically required assignment-dependent gate requires another selection-conditioned method. No survivor normalization. |
| Lifecycle | Bind the predicate set before assignment; evaluate once for the exact member/use generation after GEN completion and before ensemble admission; publish advisory diagnostics separately; immutable result with exact cause. |
...
| Policy owner | Grant Wilson. |
| Scientific sources | r0.18 owner decisions; r0.3 owner supplement; r0.4 and r0.5 owner directives; byte-bound r0.5 normative modules. |
| Required facts | Realized-state implication `A_UNC=B_resolved` and `N_admitted=N_completed=N_resolved=N_requested>0`; every resolved member positively admitted by `uncertainty_member_admission@2`; exact EQ-017 design-ignorability evidence; exact common domain; equal weights `1/N_resolved`; known target center zero; declared pre-key ideal iid `Uniform(product_h A_h)` law; exact deterministic replay identity `F(K,plan)`; all actual count/rank/orbit reports; estimator identity, target, units, representation, unavailable-or-explicit estimator uncertainty, lifecycle and provenance. |
| Exclusions | Failed design; favorable-outcome regeneration under the same method identity; partial or survivor ensemble; assignment-dependent member selection; any unavailable/incomplete/failed/unrealized member; rejected candidates; pairwise/missing-data/reweighted estimator; reciprocal, covariance, precision, physical-noise, generator-validation, or significance use. |
...
| Missing behavior | Missing authority, required fact, design-ignorability, or complete all-pass state yields `unavailable`; actual unequal counts and causes remain published, but there is no estimator, empty product, subset normalization, or divisor change. |
