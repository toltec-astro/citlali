...
OBJECTS = [
    ...
    ("SCIENTIFIC_OWNER_DECISION_LEDGER.md", "r0.6", "stable owner-decision ledger", "mixed decided/open/superseded states", "retains all 17 SODL IDs", "shared traceability"),
    ("STOCHASTIC_MODEL_AND_OBSERVED_PAYLOAD_AMENDMENT_R0.6.md", "r0.6", "stochastic/observed type amendment", "authored micro-repair", "supersedes conflated random/observed notation", "derived from exact directive"),
    ("LIFECYCLE_STATE_GRAPH_R0.6.md", "r0.6", "lifecycle-order amendment", "authored micro-repair", "supersedes incorrect publication order", "derived from exact directive"),
    ("AO001_AUTHORIZATION_MULTIPLICITY_AMENDMENT_R0.6.md", "r0.6", "authorization multiplicity amendment", "authored micro-repair", "clarifies package versus realization authority", "derived from exact directive"),
    ("AO_OWNER_DISPOSITION_PACKET_R0.6.md", "r0.6", "AO recommendation packet", "owner disposition pending", "no AO selection", "identical two-view routing basis"),
    ("OPTIMALITY_TITLE_OWNER_DISPOSITION_R0.6.md", "r0.6", "title recommendation packet", "owner disposition pending", "provisional historical title is not a selection", "identical two-view routing basis"),
    ("CONDITIONAL_SCIENTIFIC_FREEZE_PROPOSAL_R0.6.md", "r0.6", "conditional-freeze gate", "proposal unavailable pending exact dispositions", "not a scientific freeze", "owner-facing gate"),
    ("ROUTE_STATUS_R0.6.md", "r0.6", "route availability record", "authored status preflight", "supersedes active r0.5 route status", "shared status view"),
    ("SEMANTIC_CHANGE_MAP_R0.6.md", "r0.6", "micro-repair map", "authored traceability", "maps r0.5 to r0.6 only", "source/two-view parity"),
    ("OWNER_DECISION_PARITY_R0.6.md", "r0.6", "owner-direction parity record", "authored traceability", "supersedes active r0.5 parity", "source/two-view parity"),
    ("SCI-MAP_TO_SCI-FLT-MATCHED-v0.1-r0.6.md", "v0.1 r0.6", "MAP producer boundary draft", "owner review pending", "supersedes r0.5 boundary draft", "derived current boundary"),
    ("SCI-TEMPLATE_TO_SCI-FLT-MATCHED-v0.1-r0.6.md", "v0.1 r0.6", "template producer boundary draft", "owner review pending", "supersedes r0.5 boundary draft", "derived current boundary"),
    ("SCI-FLT-MATCHED_TO_SCI-NOI-v0.1-r0.6.md", "v0.1 r0.6", "NOI consumer boundary draft", "owner review pending", "supersedes r0.5 boundary draft", "derived current boundary"),
    ("SCI-FLT-MATCHED_TO_SCI-FRUIT-v0.1-r0.6.md", "v0.1 r0.6", "FRUIT producer-envelope boundary draft", "owner review pending", "supersedes r0.5 boundary draft", "derived current boundary"),
    *[(f"role_semantics/{code}_R0.6.md", "r0.6", f"{code} role-semantics draft", "unapproved, unregistered, unevaluable", "not an executable policy", "shared normative semantics") for code in ["PA", "SA", "SP", "CU", "NU", "RU", "FH"]],
    ...
    *[(f"src/common/{name}", "r0.6", f"shared normative {name.removesuffix('.tex')}", "authored micro-repair", "supersedes active r0.5 source", "imported byte-identically by both views") for name in ["notation.tex", "definitions.tex", "equations.tex", "assumptions.tex", "requirements.tex", "edge_cases.tex"]],
    ("src/scientific-rationale.tex", "v0.1 r0.6", "scientific-view source", "conditional-freeze preflight", "supersedes active r0.5 view source", "generated view over shared core"),
    ("src/engineering-conformance.tex", "v0.1 r0.6", "engineering-view source", "conditional-freeze preflight", "supersedes active r0.5 view source", "generated view over shared core"),
    ("pdf/README.md", "r0.6", "rendered-artifact index", "authored support", "active r0.6 PDF set", "describes generated views"),
    ("pdf/SCI-FLT-MATCHED-SCIENTIFIC-RATIONALE-v0.1.pdf", "v0.1 r0.6", "canonical scientific rendered view", "owner disposition pending", "supersedes active r0.5 PDF", "generated from scientific source/shared core"),
    ("pdf/SCI-FLT-MATCHED-ENGINEERING-CONFORMANCE-v0.1.pdf", "v0.1 r0.6", "canonical engineering rendered view", "owner disposition pending", "supersedes active r0.5 PDF", "generated from engineering source/shared core"),
...
def generate_source_report() -> None:
    ...
    lines = [
        ...
        "Date: `2026-09-01`",
        "",
        "Status: exact current-object byte inventory and authority-only bundle-link",
        "closure; no owner disposition, scientific freeze, implementation claim, or",
        "validation claim",
        "",
