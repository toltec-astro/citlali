#!/usr/bin/env python3
"""Create the deterministic SCI-FLT-MATCHED v0.1 r0.5 output bundle."""
...
BUNDLE_NOTE = BUNDLE_DIR / "BUNDLE_README_R0.5.md"
ARCHIVE = BUNDLE_DIR / "SCI-FLT-MATCHED-v0.1-r0.5-stage-b-output-bundle.tar.gz"
PREFIX = "SCI-FLT-MATCHED-v0.1-r0.5"
...
payload.extend(
    [
        ...
        (BUNDLE_NOTE, "BUNDLE_README_R0.5.md"),
    ]
)
if len(payload) != 70 or len({relative for _, relative in payload}) != 70:
    raise RuntimeError("expected 70 unique bundled files")
...
(Path(f"{ARCHIVE}.sha256")).write_text(f"{archive_hash}  {ARCHIVE.name}\n")
