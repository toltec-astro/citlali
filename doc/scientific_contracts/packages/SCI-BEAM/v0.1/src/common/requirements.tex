% SCI-BEAM v0.1 shared normative requirements. Document revision r0.2.
...
SCI-BEAM-REQ-007 & The fixed nominal-beam source prediction shall declare $F^{\rm TOA,nom}_{s,a}$ in top-of-atmosphere mJy per nominal beam at the fitted reference origin, plus nominal-beam identity/tensor and uncertainty; it shall not be recomputed from the fitted effective PSF. \\
...
SCI-BEAM-REQ-019 & The complete local model Jacobian of Equation~\ref{eq:model-jacobian} shall resolve every realized free parameter and its derivative method shall be validated against controlled perturbations. \\
SCI-BEAM-REQ-020 & The fit shall produce or resolve a joint covariance satisfying Equations~\ref{eq:fitcov}--\ref{eq:nuisance} where their assumptions hold and retaining all material cross terms; omitted terms shall be named. \\
...
SCI-BEAM-REQ-030 & Parameter validity shall require finite values where defined, positive-definite effective tensor, valid template normalization, declared-domain parameters, and causal support; one invalid derived use shall not erase unrelated valid quantities. \\
...
SCI-BEAM-REQ-036 & Raw fitted detector coordinates, horizon-derotated coordinates, effective per-detector angle/support/weighting/range, rotation convention, transformation uncertainty, and consistently rotated PSF tensor shall be retained. \\
...
SCI-BEAM-REQ-038 & Bracketing pointings and the science observation receiving their correction shall use the same immutable APT coordinate realization and AST rotation convention unless a separately authorized transform proves equivalence. \\
SCI-BEAM-REQ-039 & BEAM shall derive, scientifically accept, and publish \texttt{flxscale} using Equation~\ref{eq:flxscale}; source atmosphere shall be applied exactly once, no extra $H(\mathbf0)$ shall enter, and target atmosphere shall remain outside BEAM. \\
SCI-BEAM-REQ-040 & \texttt{flxscale} shall carry its TOA nominal-beam unit/role, calibrator/epoch, source model, nominal beam, passband/spectrum, source-atmosphere operator/support, amplitude/support, detector/observation, joint uncertainty, correlation scope, and acceptance state. \\
SCI-BEAM-REQ-041 & BEAM shall calculate and publish NEFD-like \texttt{sens} with Equation~\ref{eq:sens} from detector-specific trajectory-classified off-source scans and the same TOA atmosphere/calibration convention. \\
SCI-BEAM-REQ-042 & The \texttt{sens} record shall identify scan noise/baseline/bandwidth/exposure/correlation/admission policy, source-exclusion rule, invalid/partial-scan handling, minimum support, retained scan values/scatter, uncertainty, and limitations; no exact numerical estimator is authorized until the owner resolves the open decisions. \\
