...
frozen SCI-PTC v0.1/r0.5 transformed product to SCI-MAP v0.1/r0.6. It neither
...
| Response | One exact class: complete fixed-state upstream-to-PTC response, PTC full-procedure finite-difference family, whole-chain re-resolved procedure response, another named class, or typed unavailable. A fixed-state object binds source domain, PTC-sample codomain, units, parent, membership, support, coefficient state, response basis, and limitations. A full-procedure object binds perturbation family and a separate state-change record. A realized PTC-grid companion starts at MAP and is not passed through upstream response again |
...
occurrence/UID, native original occurrence, and ALIGN mapping generation. Each
positive-`e_vo` original is placed exactly once using that original's own
authorized AST coordinate and the one-hot half-open rule. An original may
...
coefficient, or formal precision.
