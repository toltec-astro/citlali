...
Owner feedback source: `SCI-JINC v0.1 — FINAL STAGE A SCOPE REPAIR BEFORE
IMPLEMENTATION-BLIND STAGE B AUTHORSHIP`, supplied `2026-08-28`, original
attachment SHA-256
`a537afc8cac35bcb1e2bf9809e74850bd2582a3f4354faf8eb0e4c7d8d3dca49`.
The raw feedback file is scope-control provenance and is not an author input.

## Applied Repairs

| Feedback area | Stage A result |
| --- | --- |
| Relation to SCI-MAP | Scope now states authorship begins after MAP freeze, JINC is a sibling alternative, consumes PTC/AST directly, consumes no MAP product, and inherits no ordinary MAP rule. |
| Eight inherited decisions | Added [`AUTHOR_DECISIONS_AND_OWNERSHIP.md`](AUTHOR_DECISIONS_AND_OWNERSHIP.md) with stable IDs, exact rules, consequences, affected objects, unavailable branches, superseded clauses, compatibility and exact source digests. |
| Analytic identity | Added [`ANALYTIC_JINC_IDENTITY.md`](ANALYTIC_JINC_IDENTITY.md). After memo intake it content-binds the complete generic two-JINC plus generalized-exponential family and adopted support/phase constraints. ODQ-102B now preserves generic `r'_a=r/s_a` semantics while typing the TolTEC numerical realization unavailable and deferring optimization. |
| TolTEC parameter disposition | Owner-approved ODQ-102B treats current `lambda_a/(45 m)`, inherited `(a,b,c)`, and mode `r_max` values as evidence only, authorizes no physical interpretation or hidden default, permits scientifically appropriate array association, and preserves exact values/history in quarantined prior-work material. |
...
| AST role | Added [`SCI-AST_TO_SCI-JINC_BOUNDARY.md`](SCI-AST_TO_SCI-JINC_BOUNDARY.md), binding frozen `SCI-AST:rtc_output_grid_coordinates@1` for the same RTC `n` and exact target WCS. |
| Admission | Added [`SCI-JINC_UPSTREAM_ADMISSION_PROFILE.md`](SCI-JINC_UPSTREAM_ADMISSION_PROFILE.md), key `SCI-JINC:upstream_admission@1`. It remains unusable pending exact owner approval and a versioned SCI-VAL registry successor; frozen VAL was not edited. |
| Coefficient-squared time | Renamed normatively to `jinc_coefficient_squared_time`; defined the squared object as dimensionless `kappa_ip`; listed every prohibited exposure/precision/significance interpretation. |
...
Unity, generated products or production behavior were inspected for this
repair. No implementation-conformity, validation, achieved-response,
covariance-fidelity, photometric-accuracy, numerical-reproducibility,
readiness or production claim is made.
