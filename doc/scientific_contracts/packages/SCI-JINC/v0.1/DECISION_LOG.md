...
| --- | --- |
| `JINC-PROCESS-D001` | Launch SCI-JINC v0.1 as the next owner-approved downstream tranche after frozen SCI-MAP v0.1/r0.7.1. |
| `JINC-PROCESS-D002` | Start from `codex/scientific-contract-library@731f821954d4321509765720c6ba1838c95eff3d` and follow the program charter, downstream roadmap, repository guidance, and recovery-first firewall. |
| `JINC-PROCESS-D003` | Limit this task to Stage A recovery, scope, decision, dossier, package scaffold, and exact proposed author-input artifacts. |
| `JINC-PROCESS-D004` | Preserve frozen ordinary SCI-MAP unchanged. JINC is a separate signed-coefficient estimator and inherits no ordinary positive-coefficient normalization, response, support, conditioning, covariance, validity, provenance, coadd, or product-availability rule. |
| `JINC-PROCESS-D005` | Recover and classify frozen SCI-MAP-002/JINC material and all later identified owner and destination-ownership decisions before any new derivation. |
...
| `JINC-PROCESS-D014` | Apply only the owner-approved ODQ-105 Stage A delta from attachment SHA-256 `c7c0760ef1a51ee549e8033806245bfdadb481a0fc986964906dfd2fbdb7b28b`: make base v0.1 observation-only, permit same-observation incremental accumulation under one exact realization/bundle identity, require a separate future boundary over complete bundles for any cross-observation combination, refresh affected controls/hashes, and return to the existing exact-byte gate without launching Stage B. |
| `JINC-PROCESS-D015` | Apply only the owner-approved ODQ-106 Stage A delta: authorize at most one independent bundle per observation/admitted-requested stable array/JINC realization/destination geometry, no placeholders or invalidity transfer for absent arrays, no merging across array/destination identities, no added per-contribution provenance, refreshed affected controls/hashes, and return to the existing exact-byte gate without launching Stage B. |
...
These are prior owner decisions, not new Stage A derivations. Their exact
sources and digests are in [`PRIOR_WORK.md`](PRIOR_WORK.md).

| Decision | Recovered approved substance |
| --- | --- |
| `SCI-MAP-002-D003-SUPPORT-001` | Retain fully populated square cache support and dual-use `r_max`; do not infer a strict radial cutoff. |
| `SCI-MAP-002-D003-SUBPIXEL-001` | Retain phase-quantized point sampling; do not reinterpret it as pixel-area integration. |
| `SCI-MAP-002-D003-CONDITIONING-001`, succeeded in part by ODQ-109 | Retain finite-positive `Q`, nonzero `C`, exact-cancellation rejection, dimensionless cancellation ratio and no unit-bearing floor. ODQ-109 replaces the summation-method/contributor-count bound with numerical error negligible against the approximately `10^-3` relative instrument-fidelity scale. |
| `SCI-MAP-002-D003-ADMISSION-001` | Require stable array identity and finite strictly positive physical parameters; preserve finite signed lobes; fail on non-finite coefficient state. |
| `SCI-MAP-002-D003-MASK-001` | Keep formal-support validity distinct from temporal support and empirical policy; empirical policy may downgrade but never promote. |
| `SCI-MAP-002-D003-COVERAGE-001` | Define `sum(c^2/f_s)` seconds as coefficient-squared effective integration time, not exposure, hits, precision, or validity. |
| `SCI-MAP-002-D003-KERNEL-001` | Define `K/C` as the processing-filtered source-template response projected through JINC with exact operator and parent identity. |
| `SCI-MAP-002-D003-PROVENANCE-001` | Require compact atomic requested/effective/resolved/realized provenance and product joins; required failure suppresses success. |

The third-successor owner acceptance confirms that the historical D003 JINC
contract was approved. Its implementation, conformity, validation, production,
and integration axes are not applied here.

## Stage A Boundary Abstraction

| Decision | Stage A treatment |
| --- | --- |
| `JINC-SCOPE-D001` | Reuse the frozen independent core once, under the proposed supersession cover; do not repeat its signed-estimator derivation. |
...
| `JINC-SCOPE-D012` | ODQ-105 makes base v0.1 observation-only without making observation an implementation-memory boundary. Same-observation samples/chunks may incrementally realize one complete bundle only under one exact observation/array/JINC realization/WCS/lifecycle identity. Cross-observation combination is neither prohibited in principle nor authorized here; any future JINC coadd requires a separately authorized boundary over complete observation bundles, with no ordinary MAP or inferred accumulator/map algebra. |
| `JINC-SCOPE-D013` | ODQ-106 permits zero through three independently identified per-array bundles for one observation, with at most one for each stable array admitted/requested under the exact realization and destination geometry. Missing, unavailable and unrequested arrays produce no placeholder and do not invalidate another bundle. Incremental contributions stay within one identity; different array/destination identities never merge. Existing plan/bundle provenance is sufficient. |
| `JINC-SCOPE-D014` | ODQ-107 fixes every produced bundle to required `N_p`, `C_p`, `Q_p`, derived `m_p` with local support/validity, and `jinc_coefficient_squared_time`. Whole-product failure suppresses the bundle; local invalid pixels remain ordinary content. No generic availability, optional/conditional-role, detailed-cause, diagnostic or provenance framework is admitted. Response/covariance/formal-weight and all other roles, including ODQ-108, are deferred until a concrete scientific use. |
...
`JINC-SCOPE-D001--D015` are proposed packet boundary statements and become
