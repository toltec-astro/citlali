...
| Cache extent | **Partly bound:** `r_max` fixes the square half-width and the first zero of the second JINC factor. | The exact integer-index equality/rounding rule remains `SCI-JINC-ODQ-109`. Evaluation also requires an authorized array-associated `s_a`, `(r_max)_a`, and pixel size; without an authorized parameter set the numerical route is unavailable. |
