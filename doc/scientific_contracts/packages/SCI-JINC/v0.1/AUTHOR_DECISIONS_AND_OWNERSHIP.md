...
`SCI-JINC-ODQ-109` is the controlling numerical-adequacy successor. It
preserves unit-invariant signed conditioning and the point-phase/square-
support operator, but supersedes the inherited demand for a machine-specific
contributor-count floating-point bound and exact owner-selected tie/bin policy.
Numerical error must instead remain negligible compared with the approximately
...
| `SCI-MAP-002-D003-SUBPIXEL-001`, as succeeded by `SCI-JINC-ODQ-109` | `subpixel_n` selects a phase-quantized, point-evaluated kernel matrix after center rounding and residual-phase binning. Increasing it refines the realized point-sampling response. | The coefficient is evaluated at one single-valued phase realization; it is not a pixel-area average. Effective `subpixel_n>=1` and total numerical error negligible against the approximately `10^-3` instrument-fidelity scale are required. Exact adequate tie/bin/cache choices are engineering realizations, not separate scientific policy. | Coefficient, square-cache placement, parameter identity and numerical adequacy. | Pixel-area integration or a numerically inadequate realization is unavailable. Bitwise identity or owner selection among adequate discrete conventions is not required. | Independent core Equation `eq:pixel-average` and its pixel-integrated convergence target; inherited exact-owner-choice wording for tie/bin/convergence. | Compatible only with point-phase evaluation, square support and the ODQ-109 accuracy principle. A changed operator or inadequate numerical realization is a successor or unavailable. |
| `SCI-MAP-002-D003-CONDITIONING-001`, as succeeded by `SCI-JINC-ODQ-109` | Require finite contributors, finite `C_p` and `Q_p`, `Q_p>0`, `C_p!=0`, and retain `rho_p=|C_p|/sum_i|omega_i kappa_ip|` as a dimensionless conditioning indicator. Exact cancellation is invalid. A finite nonzero result is usable only when numerical error is negligible compared with the approximately `10^-3` relative instrument-fidelity scale. | Finite negative `C_p` is admissible. Numerically adequate resolved cancellation remains admissible; inadequate near-cancellation is locally invalid. Stable scaling may not change the estimator. | Normalization, formal support and scientific numerical adequacy. | Unit-bearing `C` or `Q` floors, a universal `rho` cutoff, positive-denominator assumption, zero-sky substitution, silent clipping, prescribed algorithm, bitwise reproducibility or stronger precision are unavailable. | Independent core's unspecified conditioning cut and absolute floors; inherited requirement for a bound derived from realized summation method/contributor count and its serialization. | Compatible only when invariant under signal-unit/common coefficient rescaling and when numerical error is demonstrated negligible against the instrument scale. Algorithm and test construction are engineering choices. |
