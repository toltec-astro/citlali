...
Source & Owner-approved r0.3 directive and exact adjacent source-binding
register \\
Status & Owner-approved revision architecture; scientific authority not
frozen; implementation conformity and validation not assessed \\
\end{tabularx}
\vspace{0.30in}
...
redefine it. Other PTC, MAP, response, population, output, and diagnostic
profiles remain owned by their packages and are unavailable until exactly
bound. The companion Engineering Conformance Specification is the complete
formal authority; this rationale explains its scientific meaning.
...
Receive a frozen operator? & Hypothetical complete owner-bound profile used
only for illustration & Would be eligible for that application only if that
exact profile were registered, applicable, and satisfied. It could not rescue
independent exposure. \\
Remain in an output? & Hypothetical complete retention profile used only for
illustration & Would be eligible for retention only if that exact profile were
registered, applicable, and satisfied. Retention would say nothing about
fitting. \\
Display for diagnosis? & Hypothetical complete package-qualified diagnostic
profile used only for illustration & Would be eligible for display only if
that exact profile were registered, applicable, and satisfied, with
replacement provenance visible. \\
Enter PTC basis fitting? & Only the reserved profile name is known &
\textbf{Decision unavailable.} VAL cannot infer a PTC policy from finiteness
or another permission. \\
Enter PTC loading fitting? & No exact owner-bound profile & Decision
unavailable separately; basis and loading fits are different questions. \\
Enter coefficient/QC estimation? & No exact owner-bound profile & Decision
unavailable; coefficient/QC is not an alias for estimator fit. \\
...
\RationaleBox{Why one flag fails.}{This occurrence is finite, retained,
directly replaced, ineligible for independent exposure, potentially eligible
for a non-independent use only under an exact future registered and applicable
profile, and undecidable for unbound uses.
A single good/bad bit loses the scientific question, owner, and reason.}
...
to admit direct replacement as independent exposure is itself invalid; the
result is decision unavailable with a policy-invalid reason, never eligible.
