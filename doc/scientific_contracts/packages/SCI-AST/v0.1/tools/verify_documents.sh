...
export SOURCE_DATE_EPOCH=1787400000
...
test "$boundary_sha" = 04357d36b302d607b95950f529044e178deb2528d0c6f656d90da93067a5da36 || {
  echo "unexpected final shared boundary digest: $boundary_sha" >&2
  exit 1
}

for required_record in \
  "$package_dir/NOTATION_AND_SYMBOL_CHANGE_MAP.md" \
  "$package_dir/REQUIREMENT_EQUATION_PREDICTION_CHANGE_MAP.md" \
  "$package_dir/CROSSWALK.md" \
  "$package_dir/OWNER_DECISION_REGISTER.md" \
  "$package_dir/SOURCE_MANIFEST.md" \
  "$package_dir/PDF_VISUAL_QA.md" \
  "$package_dir/FORMAL_PARITY_REPORT.md" \
  "$package_dir/BOUNDARY_IDENTITY_AND_EQUALITY_REPORT.md"; do
  test -s "$required_record" || {
    echo "missing r0.3 delivery record: $required_record" >&2
    exit 1
  }
done

grep -F 'SCI-AST:rtc\_output\_grid\_coordinates@1' \
  "$common_dir/requirements.tex" >/dev/null || {
  echo "missing stable RTC-grid coordinate role" >&2
  exit 1
}
grep -F 'AST-OWNER-Q008} is closed' "$common_dir/assumptions.tex" >/dev/null || {
  echo "AST-OWNER-Q008 ordinary-chain disposition is not closed" >&2
  exit 1
}
grep -F '\boldsymbol t=\eta_1\boldsymbol e_1+\eta_2\boldsymbol e_2' \
  "$common_dir/equations.tex" >/dev/null || {
  echo "complete spherical tangent-vector oracle is absent" >&2
  exit 1
}
if grep -F 'I^{\rm geom}_{pi}' "$common_dir"/*.tex "$source_dir"/*.tex; then
  echo "general pre-MAP I^geom candidate remains in canonical sources" >&2
  exit 1
fi
grep -F 'double-derotate' "$common_dir/definitions.tex" >/dev/null || {
  echo "geometry representation/application-count guard is absent" >&2
  exit 1
}
