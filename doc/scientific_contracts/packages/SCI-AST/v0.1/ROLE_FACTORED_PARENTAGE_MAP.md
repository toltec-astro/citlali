# SCI-AST v0.1 Stage B r0.3 Role-factored Parentage Map
...
| RTC-output-grid coordinate (`SCI-AST:rtc_output_grid_coordinates@1`) | `(appropriate direction/tangent/pixel role, exact RTC product, plan, grid, stable output n, representative stable ALIGN slot, output time, phase/delay, segment, decimation, support, response and correction state)` | AST owns realization for every ordinary science product whose numerical signal is represented on an RTC grid. Missing RTC facts affect only the dependent RTC role/response/delegated deposition, not unrelated ALIGN-grid roles. |
