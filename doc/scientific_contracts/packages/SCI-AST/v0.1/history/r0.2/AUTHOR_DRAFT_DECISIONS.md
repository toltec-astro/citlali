...
| `SCI-ALIGN_TO_SCI-AST_BOUNDARY.md` | `6b8e3c360c9933a882bd9aa2d25dbce4365a5e568ddca630dbc1c2e3a013ecc2` |
...
For the Stage B r0.2 targeted revision, the sole new scientific directive was
the supplied `pasted-text.txt` with SHA-256
`15f751388d775920872228618c4bf073fcdb7fe5528fdc006789ae76a601062b`.
The final shared boundary installed by the ALIGN-led coordinator is
`SCI-ALIGN_TO_SCI-AST v0.1/r0.1`, SHA-256
`359444fec10f35a3c7ab6d59c5d8d127d24f07dfce3f33590eac6268d07489cf`.
It was not independently edited by the AST author.
...
| `SCI-AST-AUTH-D016` | Name geometry-only candidate incidence/stencil/support `I^geom_pi` and reserve `G_pi` for a complete exact MAP-owned deposition request. | Prevents geometry from implying estimator contribution, normalization, conservation, MAP validity, response, or covariance. |
...
| The TAN/WCS “projection” term could be confused with MAP sample deposition and `G_pi`. | Definitions and requirements reserve TAN/WCS astrometric projection for AST and sample-to-pixel deposition/gridding for MAP. Geometry-only candidate incidence is separately named `I^geom_pi`; exact `G_pi` materialization is a delegated service only for a complete MAP-owned request with all parents. |

## Preserved Owner Questions And Blocked Claims

No answer was inferred for `AST-OWNER-Q001`–`AST-OWNER-Q008`. Their exact text
and states are preserved in `OWNER_DECISION_REGISTER.md`. The resulting
unavailable outputs/claims are:
...
| `AST-OWNER-Q008` | Unconditional family use of `theta^RTC_dn` or response-aware RTC-grid coordinates. |
