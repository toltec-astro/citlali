# SCI-AST v0.1 — Stage B r0.2 Targeted Draft Crosswalk
...
| `SCI-ALIGN_TO_SCI-AST_BOUNDARY.md` (`SCI-ALIGN_TO_SCI-AST v0.1/r0.1`) | notation, definitions, requirements, edge cases | `REQ-006`–`012` (exact profile/transfer, stable slot `s`, paired-signal provenance and five causes); `REQ-013`–`022` (producer-selected pointing support, current observing-state separation, interpolation within support and no extrapolation); `REQ-056`–`060` (role validity, dependency-limited failure and layered atomicity); `REQ-073` (role-factored ALIGN-grid parents); `REQ-080`–`083` (AST/MAP projection ownership, candidate `I^geom_pi`, exact MAP-owned `G_pi`); `REQ-084`–`088` (parent/version and four-stage provenance); `PRED-004`–`010`, `015`, `018`, `025`, `030`, `035`–`040`, `049` |
...
| `AST-OWNER-Q008` | `SCI-AST-ASM-012` | `REQ-074`–`079`, `082` | Unconditional RTC-grid family use |
