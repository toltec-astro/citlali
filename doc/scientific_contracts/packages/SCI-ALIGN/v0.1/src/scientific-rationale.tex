...
\hypersetup{
  ...
  pdftitle={SCI-ALIGN v0.1 Scientific Rationale r0.3},
...
\fancyhead[R]{Stage B targeted draft r0.3}
\fancyfoot[C]{\thepage}
\raggedbottom
...
separate.  The detector-reference occurrence time \(t_s\) is the exact
grid/time identity, not another scalar observing-state field subject to
interpolation.  The observing-state family describes the science occurrence
at that exact time: aligned boresight, current elevation, required azimuth,
and any additional registered values needed by AST geometry or field rotation.
ALIGN preserves the producer's unit, frame, topology, support, and meaning.  A
producer telescope timestamp may remain native support metadata, but it is
not a competing current-sample time.
...
physical acquired $e^{\rm acq}_{sd}$\\
valid-original $e^{\rm vo}_{sd}$};
\draw[arr,aligngold] (5.0,0.2) -- (facts.west);
\end{tikzpicture}
\caption{One stable target slot carries distinct timing and product-state
facts.  Physical acquired exposure records attributable original integration
even when its payload is invalid; valid-original exposure is the usable local
subset.  Synthesized, missing, and unoccupied support add zero acquired
exposure, and later guards or use policy do not rewrite acquisition.}
...
Physical acquired exposure also follows stable source identity rather than
window membership.  Valid-original, guarded, retained, and use-qualified
exposure remain separate later facts.  Overlapping membership is rejected
unless a successor explicitly defines it and prevents double counting.
...
families; origin, support, physical acquired and valid-original exposure,
intervals, mapping validity, response,
