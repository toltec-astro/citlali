...
Status: Stage B targeted draft r0.2 pending scientific-owner approval
...
paired physical KID readout coordinates.
...
| Native and assigned time relation | Native interface identity; native row/occurrence identity; producer-declared native acquisition/integration support; corrected native event time `t^ref` when available; nominal assigned slot time; assignment residual; offset-record identity | Nominal slot time, corrected native event time, residual, native support, nominal cell support, and acquired exposure remain distinct quantities. Physical-time meaning is no stronger than producer authority. |
...
| Aligned boresight and current observing-state fields | Boresight direction; current elevation; current azimuth where required; current science-sample time; every other declared telescope/observing-state field needed by AST geometry or field rotation; stable field identity and registry version; unit; unchanged producer frame; topology; declared per-field operator; exact source mapping and support; origin; validity; uncertainty availability | These fields describe the science occurrence itself on the detector-reference grid. ALIGN maps declared producer meaning; AST receives it without reinterpretation. Field rotation uses these aligned occurrence-local fields. Exact telescope/boresight/HWPR registry entries remain owner questions rather than inferred defaults. |
...
| Origin and synthesis | Original, synthesized, unavailable, original-invalid, and guarded state; continuity availability; source-relation identity; acquired-exposure contribution | Origin is never upgraded. Synthesized support has zero added acquired exposure and is never an original independent occurrence. Coordinate-specific invalidity does not authorize filling from its pair. |
...
current time/elevation/required azimuth, and other observing-state fields at
the science occurrence. Separately, the pointing-support producer selects the
