...
| D15 | **Evidence tooling ready 2026-07-23:** fruit-loop execution has activation validation and a hard iteration bound, but no implemented scientific convergence statistic; a valid active feedback request therefore runs to `max_iters`. The offline NGC4449 study measures per-array raw-map, support, weight, and learning-state stability, but the five-iteration sequence supplies only one complete late pass and does not justify early stopping. | Scientific owner and Citlali engineering | **Triggered:** the full NGC4449 run demonstrated material cost from iteration policy | Approve an estimator, tolerance, minimum-iteration and consecutive-pass rule, and provenance fields from representative state-complete sequences; demonstrate early stopping without changing accepted final science beyond the successor profile |
| D16 | **Implementation complete; one Unity gate remains:** TolPROJ derives refactor Slurm CPUs from the preserved runtime config, synchronizes explicit overrides, and validates before submission. Citlali independently discovers Slurm/affinity/hardware availability, caps oversubscribed thread plans with one warning, continues, and records requested/available/effective/realized state in runtime provenance V2. The matching Unity smoke passed at `d339053cc`: six requested, affinity-available, and effective threads; valid V2 provenance; no issues; exact non-profile products against the predecessor. | TolTECA runtime owner and Citlali engineering | **Triggered:** the NGC4449 run requested 16 OpenMP threads inside an 8-CPU allocation | Focused and full local gates plus the matching Unity smoke pass; one intentional direct-submit mismatch must demonstrate successful capping, one warning, completion, and V2 provenance before this item is marked closed |
