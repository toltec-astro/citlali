...
| D09 | Compact config is translation tooling, not production authoring authority. The project owner reopened the user-facing authoring work as Phase 4.1 on 2026-07-16. | TolTECA/config owner and Citlali engineering | **Triggered:** build a clear numbered-YAML kit for point, OOF, Beammap, and science | Four ready-to-copy mode kits and hermetic real TolTECA overlay tests cover ordering, precedence, expert overrides, list replacement, null/deletion, aliases, unknown keys, and multiple reduction steps; accepted low-level equivalence and mode smoke gates pass |
...
| D15 | Fruit-loop execution has activation validation and a hard iteration bound, but no implemented scientific convergence statistic; a valid active feedback request therefore runs to `max_iters`. | Scientific owner and Citlali engineering | **Triggered:** the full NGC4449 run demonstrated material cost from iteration policy | Approve an estimator, tolerance, minimum-iteration rule, and provenance fields; demonstrate early stopping without changing accepted final science beyond the successor profile |
| D16 | Runtime thread requests are not reconciled with the enclosing Slurm CPU allocation, so OpenMP/FFTW settings can oversubscribe a job. | TolTECA runtime owner and Citlali engineering | **Triggered:** the NGC4449 run requested 16 OpenMP threads inside an 8-CPU allocation | One allocation-aware policy either derives all thread counts from granted CPUs or fails conflicting explicit requests before reduction; effective thread counts are recorded and a cluster smoke test passes |
...
- D06-D08 and D10 require scientific or upstream ownership before
  implementation. D09 proceeds only through the bounded Phase 4.1 plan.
