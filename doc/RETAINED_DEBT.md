...
| D15 | **Discussion candidate recorded 2026-07-27:** fruit-loop execution has activation validation and a hard iteration bound, but no implemented scientific convergence statistic; a valid active feedback request therefore runs to `max_iters`. The quality-stratified 108-observation collection is in progress. The first 50 complete Stage B observations motivate an offline candidate with a 3% kernel-normalized amplitude limit, no evaluation before iteration 6, and two consecutive all-array passes, but the rule remains unapproved pending the full population, separate PSF/map/noise limits, and stopped-versus-final comparisons. The historical pointing-table `sig2noise` dynamic-range value is explicitly excluded from convergence. | Scientific owner and Citlali engineering | **Triggered:** the full NGC4449 run demonstrated material cost from iteration policy | Approve an estimator, separate scientific tolerances, minimum-iteration and consecutive-pass rule, and provenance fields from representative state-complete sequences; demonstrate early stopping without changing accepted final science beyond the successor profile |
