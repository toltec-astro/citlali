...
| D16 | **Implementation complete locally 2026-07-23; Unity closeout pending:** TolPROJ derives refactor Slurm CPUs from the preserved runtime config, synchronizes explicit overrides, and validates before submission. Citlali independently discovers Slurm/affinity/hardware availability, caps oversubscribed thread plans with one warning, continues, and records requested/available/effective/realized state in runtime provenance V2. | TolTECA runtime owner and Citlali engineering | **Triggered:** the NGC4449 run requested 16 OpenMP threads inside an 8-CPU allocation | Focused and full local gates pass; one matching Unity smoke and one intentional direct-submit mismatch demonstrate successful capping and V2 provenance before this item is marked closed |
