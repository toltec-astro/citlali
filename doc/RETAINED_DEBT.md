...
| D17 | **Triggered 2026-07-29:** Citlali copies raw `Header.Toltec.AdcSnapData` into diagnostics but does not validate signed 12-bit rail occupancy, low headroom, or beginning-to-end change before reducing the affected network. The producer-confirmed `[2,4096]` boundary ordering and `[-2048,2047]` count domain are now explicit. NGC4449 pointings show chronic beginning-and-end clipping in nw9, sparse rail contact or low headroom in nw3/nw4, and no common ADC-saturation onset for the broader late multi-network map pathology. No automatic warning, exclusion, or failure threshold is approved. | TolTEC readout owner, scientific owner, and Citlali engineering | **Triggered:** real pointing data contain persistent ADC clipping that is scientifically relevant but not sufficient to explain every affected map | A cold-boundary validator records per observation, network, and file boundary the extrema, RMS, peak-to-peak utilization, exact rail counts/fraction, and near-rail occupancy; outputs carry explicit validity/severity; approved thresholds distinguish informational low headroom, actionable clipping, network exclusion, and whole-reduction failure; clean, marginal, and clipped fixtures plus affected-mode validation pass |
| D18 | The `SCI-MAP-001` successor keeps ordinary `weight_I` as a nonprecision gridding/normalization coefficient. Marginal precision, covariance, coadd uncertainty, GLS, and statistical-significance interpretations remain unavailable without the applicable `SCI-PTC-001` and noise-package evidence. The complete v1 F010 bundle remains unavailable for JINC and detector-grouped products. `SCI-MAP-002` now approves a separate signed JINC `N/C`, conditional formal `C^2/Q`, formal-support, coverage-seconds, kernel-response, and compact-provenance contract; its exact application repair still requires Unity evidence and independent exact-SHA re-audit before disposition. F009/F010 implementation acceptance remains pending an independent exact-repair-SHA re-audit. F012 is owner-accepted only for the bounded external claims in the 2026-08-05 amendment; its missing operational and internal-reconstruction lanes remain limitations rather than authorization for stronger claims. | Scientific map/product owner with PTC, noise, and JINC owners | An upstream audit approves the required precision/covariance evidence; the exact `SCI-MAP-002` repair is ready for Unity evidence and independent disposition; a versioned consumer requires one of the unavailable F010 products; or the exact MAP-001 repair is ready for independent disposition | Separately approved versioned successors state the applicable estimator, coefficient/covariance meaning, grouping/method availability, products, provenance, and consumer restrictions; affected local and external gates pass; and the responsible audits record their dispositions |
| D19 | WP-7 native occurrence support provisionally treats reconstructed event time as the integration center and uses raw `AccumLen / FpgaFreq` as duration. This is an explicit engineering assignment, not a producer-timestamp semantics claim; calibration remains pending. | TolTEC readout/scientific owner and Citlali engineering | The native timestamp calibration or producer timing relation becomes available | Replace or correct the assignment with the calibrated producer relation, version the affected support contract, rerun representative paired-data and chunk-invariance gates, and disposition any scientific differences |
...
- D17 authorizes schema and evidence collection. It does not authorize a
  detector/network exclusion or reduction-failure threshold until the readout
  and scientific owners approve the action policy against representative
  clean, marginal, and clipped observations.
- D18 does not authorize GLS, covariance regularization, a precision or
  significance claim, JINC parameter/filter/footprint changes,
  detector-group algorithm changes, or silent F010 synthesis. Each requires
  its named owner decision, versioned contract, and affected-package evidence.
- D19 does not permit the provisional midpoint interval to be described as a
  measured or producer-authoritative timing relation. It does permit bounded
  identity-route execution and comparison while calibration is pursued.
