...
Frozen handoff manifest & Record canonical registry commit, manifest path,
SHA-256, and every inbound handoff ID/review phase.\\
Pre-core handoff exposure & List only coordinator-approved
\code{pre_core_authority} records opened before the freeze.\\
First post-core handoff exposure & Record timestamp, handoff ID, and
confirmation that opening followed the independent-core freeze.\\
\bottomrule
\end{longtable}
...
Verify the frozen dispatch manifest and disposition every inbound record.
Submission claims remain the source package's bounded evidence; this audit
owns only its recipient disposition. Propose one separate outbound record per
target package and do not edit the canonical registry from this branch.

\begin{longtable}{L{0.17\linewidth}L{0.14\linewidth}L{0.15\linewidth}L{0.18\linewidth}L{0.26\linewidth}}
\toprule
Handoff ID & Source/target & Review phase & Status/action & Audit effect and
rationale\\
\midrule
\endfirsthead
\toprule
...
\midrule
\endhead
TO-COMPLETE & TO-COMPLETE & \code{pre_core_authority} or
\code{post_core_evidence} & TO-COMPLETE & Link the affected finding,
dependency, contract section, restriction, test, or re-audit trigger.\\
\bottomrule
\end{longtable}

For every outbound proposal state the exact claim, supported inference,
explicit limitations, evidence identities/hashes, requested action, and
related handoff IDs. A handoff cannot close a finding or authorize production
by itself.
...
\section{Numerical proportionality and execution cost}

List every proposed numerical or external-evidence study and classify whether
repetition materially exposes compute, memory, storage, scheduling, scarce
data, or human operational cost. For a non-costly study, give the measured or
bounded basis. For a costly study, execution remains held until the exact
artifacts required by \code{FRAMEWORK-NUM-001} pass the model-free launch gate.

\begin{longtable}{L{0.16\linewidth}L{0.13\linewidth}L{0.17\linewidth}L{0.19\linewidth}L{0.23\linewidth}}
\toprule
Study ID & Cost class & Condition register & Preflight/certificate & Raw/evaluator salvage boundary\\
\midrule
\endfirsthead
\toprule
Study ID & Cost class & Condition register & Preflight/certificate & Raw/evaluator salvage boundary\\
\midrule
\endhead
complete & costly or not costly & path/digest or N/A & path/digest/status or held & complete\\
\bottomrule
\end{longtable}

Every aborting numerical tolerance must have a derived numerical bound or a
quantified mapping into the final scientific metric. Exact identity is used
only for objects with discrete or byte identity. Engineering diagnostics are
warning-only; an impact analysis that establishes identity, numerical-
correctness, or scientific authority requires explicit reclassification as
Class A, B, or C. Record separate validity states for raw model output,
parser/admission products, evaluator products, and the final scientific
decision.
...
Include a YAML fragment matching \code{doc/audits/audit-ledger.yaml}, including
\code{incoming_handoffs} and \code{outgoing_handoffs}. It is a proposal for
coordinator review, not permission to modify the canonical ledger or handoff
registry from a parallel audit branch.
