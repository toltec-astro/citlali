...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{TolTEC Technical Memo}
\fancyhead[R]{\thepage}
...
% Memo metadata
% -------------------------------------------------
\newcommand{\memotitle}{Noise Estimation and Signal-to-Noise Products in Citlali:\\
Definitions, Correlated Noise, and Proposed Data Products}
\newcommand{\memonumber}{TolTEC-TM-2026-XXX}
\newcommand{\memoauthor}{G. Wilson \& Codex}
\newcommand{\memoaffiliation}{Department of Astronomy, University of Massachusetts Amherst}
\newcommand{\memodate}{2026-05-01}
\newcommand{\memostatus}{Internal Draft}
\newcommand{\memoversion}{v0.1}
\newcommand{\memoproject}{TolTEC / Citlali}
\newcommand{\memoreviewers}{TolTEC Data Reduction Working Group}
\newcommand{\memodistribution}{Internal TolTEC Collaboration}
\newcommand{\memokeywords}{Citlali, noise, jackknife, weights, signal-to-noise, correlated noise}
\newcommand{\memoabstract}{This memo defines the intended meaning of Citlali noise-related
products and proposes a unified treatment for formal weights, empirical
jackknife noise realizations, raw and filtered signal-to-noise maps, and
estimator-specific source-flux uncertainties. The central recommendation is to
keep full jackknife realizations as the empirical noise ensemble, compute
calibrated summary products from that ensemble, and make all science
signal-to-noise products explicit about the estimator they represent.}

\newcommand{\projectlogo}{%
  \IfFileExists{assets/toltec_logo.png}{
    \includegraphics[width=0.2\textwidth]{assets/toltec_logo.png}
  }{
    {\small\textbf{TolTEC}}
  }
}
...
\begin{titlepage}
  ...
  \begin{center}
    ...
    {\Large\bfseries TolTEC Technical Memo\par}
    \vspace{0.6cm}

    {\LARGE\bfseries \memotitle\par}
    \vspace{1.0cm}

    {\large \memoauthor\par}
    {\normalsize \memoaffiliation\par}

    \vspace{0.8cm}

    {\normalsize \memodate\par}
  \end{center}

  \begin{center}
    \renewcommand{\arraystretch}{1.3}
    \begin{tabularx}{0.92\textwidth}{>{\bfseries}p{0.28\textwidth} X}
      \toprule
      ...
      Project          & \memoproject \\
      Author           & \memoauthor \\
      Reviewers        & \memoreviewers \\
      Distribution     & \memodistribution \\
      Keywords         & \memokeywords \\
      \bottomrule
    \end{tabularx}
  \end{center}
...
\[
  \texttt{noise\_maps.write\_realizations = true/false}.
\]
...
\section{Citlali Configuration}

The current implemented Citlali noise-map configuration is:
...
Here \texttt{write\_realizations} controls whether the full jackknife noise
realization maps are written as separate FITS files. This key replaces older
or proposed names such as \texttt{write}, \texttt{write\_coadd}, or
\texttt{write\_noise\_realizations}; those names are not part of the current
implemented Citlali configuration. If \texttt{coadd.enabled=true}, written
realization FITS files are coadd noise maps. If coadding is disabled, written
realization FITS files are per-observation noise maps.

The \texttt{products} block controls the summary noise products computed from
the jackknife ensemble. With \texttt{products.enabled=true}, Citlali computes
the jackknife variance, formal-to-empirical weight calibration, empirical
weights, and explicit pixel signal-to-noise products. With
\texttt{products.apply\_empirical\_weights=true}, the primary output
\texttt{weight} HDUs are the empirical weights, while the original mapmaker
weights are preserved in \texttt{weight\_formal} HDUs.

Future estimator-specific configuration may add blocks like the following, but
this is not yet the implemented schema:
