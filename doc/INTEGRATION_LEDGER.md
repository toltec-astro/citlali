...
The application landmarks and their four independent status axes are routed
...
| --- | --- | --- | --- | --- |
| `APP-CANON-001` | Canonical application integration on `codex/refactor-mainline` at exact head `cb3d568c701217ee0248c77f6dccd0bab7deef31` | Normal accepted application work lands only through reviewed canonical ancestry | Active authority; not a claim that every topic branch is integrated | Ordinary affected-behavior gates and owner-controlled integration |
| `WP7-GOV-001` | WP-7.1 governance reconciliation on `codex/wp7-governance-reconciliation`, based exactly on `cb3d568c...` | This is the only active WP-7 application integration work order | G0--G3 assembled and locally verified; no successor application implementation | Commit the package, then require owner review and an explicit resume before G4 replay |
| `WP7-DIV-001` | Preserved divergent implementation/evidence lane `codex/wp7-rtc-fixed-decimation-authority` at immutable head `49fe73e757daa1885cd23127e8441cba47e648d2` | No discard, rebase, push, or new application commit while the governance hold is active; uncommitted work is not an identity | Held at a safe boundary with all committed and dirty work preserved | Eligible only for bounded reviewed replay after `WP7-GOV-001` closes |
...
| Spack build-modernization integration | `codex/integrate-spack-build-modernization`, based on exact mainline checkpoint `a36abaebfb82d503b113de0cf4c1c6e0f6dcffc3` and merging exact build-lane head `d9843e85ed87ba9ac8c42d8cc21f997dacbe1046` | Historical source lane for the Tula CMake/Spack build, package-consumer, provenance, release-contract, and timing work | Preserve the source-lane evidence; current release composition and same-SHA platform closure follow the canonical status document | Integrated into the Native Integration Baseline; source lane preserved, remaining Phase 5 release gates open |
