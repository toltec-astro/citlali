...
| WP-7 timestream integration candidate | `codex/wp7-timestream-integration-candidate`, preserving exact Unity-tested application commit `3ebc2a67fc32bad69759ff45638484efabf91773`; bounded cleanup `aa85a2287`; audit-tool repair `ff7899668` | Carry completed Stage 7 152390 evidence, retire only pre-censused inactive sources, admit current campaign evidence to the reduction auditor, and preserve a classified ref/worktree retirement record | Fast-forward only after the complete local candidate gates; retain `3ebc2a67f` as the exact Unity science identity; do not import Conan adaptation or divergent science-contract/audit-evidence histories; owner controls push and ref retirement | Local integration candidate; `origin/codex/refactor-mainline` at `46ad23888` is 71 commits behind exact tested `3ebc2a67f` with zero divergence; no push, ref deletion, successor implementation activation, or science-ledger entry authorized; general successor architecture is governed separately by ADR 0014 |
| WP-7 timestream successor baseline | `codex/wp7-timestream-successor-baseline`, based on exact integration candidate `a36abaebfb82d503b113de0cf4c1c6e0f6dcffc3` | Preserve the repository-native ADR, implementation baseline, and exact evidence authority manifest for the bounded subsystem succession | Documentation-only baseline; do not combine it with Conan adaptation or successor implementation; owner controls integration, and future implementation starts from the accepted application authority | Accepted architecture baseline; implementation not started; no production activation, science-ledger, or validation-epoch change |
...
| Conan 2 adaptation | Governing name `codex/conan2-adaptation`; observed active worktree authority `codex/build-adaptation` at `d9843e85ed87ba9ac8c42d8cc21f997dacbe1046`, five commits ahead of `origin/codex/build-adaptation` | Port the full application into the accepted Tula/Conan 2 architecture | Keep separate from application source cleanup; return only after all build-integration gates pass; retain both names until the owner reconciles the discrepancy | Active in `/Users/gwilson/GitHub/citlali-refactor-build`; historical `origin/codex/conan2-adaptation` remains retained |
