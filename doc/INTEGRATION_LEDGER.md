...
| Spack build-modernization integration | `codex/integrate-spack-build-modernization`, based on exact mainline checkpoint `a36abaebfb82d503b113de0cf4c1c6e0f6dcffc3` and merging exact build-lane head `d9843e85ed87ba9ac8c42d8cc21f997dacbe1046` | Reconcile the complete current application with the accepted Tula CMake/Spack build, package-consumer, provenance, release-contract, and timing work | Preserve application/scientific authority while importing the build lane; run both established and Spack local gates; require an exact-SHA Unity point A/B before mainline promotion | Active local integration candidate; no push or Unity action by Codex |
