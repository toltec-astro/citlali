...
class OperationalTrial(unittest.TestCase):
    def setUp(self):
    ...
    def test_declared_budget_and_failed_solver_cannot_supply_feedback(self):
        e=self.make();e.D=e.D&(np.hypot(self.x,self.y)<=60)
        z=self.noise+np.where(e.D,100*np.exp(-((self.x-7)**2+(self.y+3)**2)/40),0)
        def fake(fun,x,**kw):
            self.assertEqual(kw['options']['maxiter'],3000)
            self.assertEqual(kw['options']['maxfun'],30000)
            return SimpleNamespace(x=np.ones_like(x),success=False,message='iteration limit',nit=3000,nfev=3001)
        with patch('candidate.minimize',fake):u,rec,_=e.infer(z)
        self.assertFalse(rec['available']);self.assertFalse(rec['admitted'])
        self.assertTrue(np.all(u==0));self.assertTrue(np.any(e.last_trial>0))
