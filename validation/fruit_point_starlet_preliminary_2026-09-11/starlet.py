"""One frozen experimental starlet estimator. No production or truth imports."""
import time
import numpy as np
...
class Estimator:
    ...
    def infer(self,z):
        ...
        elif not np.isfinite(rec['outer_MAD']) or rec['outer_MAD']<=0:
            # The approved procedure does not supply a zero-scatter fallback.
            # A noiseless unavailable solve is reported, never repaired by
            # selecting a new numerical normalization after seeing outcomes.
            rec.update(reason='nonpositive_outer_MAD_for_declared_solver_scaling')
        else:
            scale=rec['outer_MAD'];target=Y/scale;weights=np.where(omega,(scale/sigma)**2,0).reshape((5,)+self.shape)
