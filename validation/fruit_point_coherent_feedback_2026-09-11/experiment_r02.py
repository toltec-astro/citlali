...
def coherent_start(residual,x,y,width):
    # Data-only matched Gaussian score locates a numerical start. No truth,
    # expected flux, nominal centroid or nominal beam enters this operation.
    xs=np.unique(x);ys=np.unique(y);dx=float(np.min(np.diff(xs)));dy=float(np.min(np.diff(ys)))
    ix=np.rint((x-xs[0])/dx).astype(int);iy=np.rint((y-ys[0])/dy).astype(int)
    shape=(len(ys),len(xs));canvas=np.zeros(shape);mask=np.zeros(shape)
    canvas[iy,ix]=residual;mask[iy,ix]=1.
    sx=width/FWHM/dx;sy=width/FWHM/dy
    ky,kx=np.mgrid[-int(np.ceil(4*sy)):int(np.ceil(4*sy))+1,-int(np.ceil(4*sx)):int(np.ceil(4*sx))+1]
    K=np.exp(-.5*((kx/sx)**2+(ky/sy)**2))
    numerator=signal.fftconvolve(canvas,K,mode='same');denominator=signal.fftconvolve(mask,K*K,mode='same')
    allowed=(mask!=0)&(np.abs(xs[None,:])<80)&(np.abs(ys[:,None])<80)&(denominator>1e-12)
    score=np.full(shape,-np.inf);score[allowed]=numerator[allowed]/np.sqrt(denominator[allowed])
    iy0,ix0=np.unravel_index(np.argmax(score),shape)
    return numerator[iy0,ix0]/denominator[iy0,ix0],xs[ix0],ys[iy0]
...
def fit_source(z,x,y,domain):
    ...
    results=[]
    for w in [6.,18.,42.]:
        amp0,cx0,cy0=coherent_start(r,xx,yy,w)
        p=np.array([amp0,cx0,cy0,np.log(w),np.log(w),.2,*b])
...
def verify_freeze():
    f=json.loads((HERE/'FREEZE_R0.2.json').read_text())
    for row in f['files']:
