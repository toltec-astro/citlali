...
class Run:
    ...
    def check(self,stage=None):
        ...
        require(not psutil.Process().children(recursive=True),'Unexpected child process in one-process campaign')
...
def main():
    ...
    write_json(out/'RUN_START.json',dict(started_utc=datetime.datetime.now(datetime.UTC).isoformat(),decision=settings['decision'],proposal_commit=settings['proposal_commit'],source_rows=source_rows,environment=dict(python=sys.version,numpy=np.__version__,scipy=scipy.__version__,netCDF4=netCDF4.__version__,platform=platform.platform(),pid=os.getpid()),thread_limits={k:os.environ[k] for k in ('OMP_NUM_THREADS','OPENBLAS_NUM_THREADS','MKL_NUM_THREADS','VECLIB_MAXIMUM_THREADS')},inputs_opened=False))
