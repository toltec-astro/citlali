...
CONFIG=dict(spacing_arcsec=3.,fwhm_arcsec=4.,center_radius_arcsec=94.,lambda_value=.01,screened_floor=.01,
...
class Basis:
    ...
    def infer(self,m,R):
        ...
        inference_seconds=time.monotonic()-start;start=time.monotonic();diagnostic=self.concentration(a,admitted)
        diagnostic_seconds=time.monotonic()-start
        return np.where(self.D,model,0) if admitted else np.zeros_like(model),a,dict(reason=reason,admitted=bool(admitted),scale=R,
            heldout_scores=scores,fold_cosine=cos,background=beta,full_solver=full,fold_fits=fits,concentration=diagnostic,
            inference_seconds=inference_seconds,diagnostic_seconds=diagnostic_seconds)
