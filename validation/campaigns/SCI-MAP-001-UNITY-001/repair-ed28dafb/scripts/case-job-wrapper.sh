...
if [[ $# -ne 8 ]]; then
  echo "usage: case-job-wrapper.sh CASE_ROOT EVIDENCE_DIR MASTER_BINARY EXPECTED_MASTER_SHA SNAPSHOT EXPECTED_SNAPSHOT_SHA PRE_SUBMIT_INTEGRITY_MANIFEST RECONSTRUCTION_AUTHORITY_MANIFEST" >&2
  exit 64
fi
...
wrapper_outputs=(
  ...
  pre-run-sha256.txt integrity-manifest.sha256 reconstruction-manifest.sha256
...
reconstruction_manifest_sha=$(sha256sum "$reconstruction_manifest" | awk '{print $1}')
sha256sum -c "$integrity_manifest"
sha256sum -c "$reconstruction_manifest"
...
printf '%s  %s\n' "$reconstruction_manifest_sha" "$reconstruction_manifest" \
  > "$evidence_dir/reconstruction-manifest.sha256"
...
printf '%s  %s\n' "$reconstruction_manifest_sha" "$reconstruction_manifest" | sha256sum -c -
sha256sum -c "$integrity_manifest"
sha256sum -c "$reconstruction_manifest"
