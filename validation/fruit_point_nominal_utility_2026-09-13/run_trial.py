...
def make_estimators(data):
    ...
    estimators, records = [], []
    for a in range(3):
        e = nominal.Estimator(data.x, data.ygrid, data.shape, data.S[a], data.D[a], data.O[a], data.Q[a])
    ...
    def trajectory(self,data,parent,name,arm,estimators,truth,definition):
        ...
        try:
            for k in range(7):
                ...
                if self.cleaning_calls > 238:
                    raise TrialLimit("registered cleaning-call ceiling")
