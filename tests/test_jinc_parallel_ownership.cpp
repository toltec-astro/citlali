...
mapmaking::MapBuffer make_map(const std::string &name, bool with_signal,
    ...
    if (with_signal) {
        citlali::pipeline::allocate_map_matrices(
            map, kMaps, true, with_kernel, with_coverage, false, {}, true);
    }
    if (with_noise) {
...
void expect_map_exact(const mapmaking::MapBuffer &lhs,
    ...
    expect_matrix_vector_exact(
        lhs.jinc_products.denominator_sum_abs,
        rhs.jinc_products.denominator_sum_abs);
    ASSERT_EQ(lhs.jinc_products.contributor_count.size(),
              rhs.jinc_products.contributor_count.size());
    for (std::size_t i = 0; i < lhs.jinc_products.contributor_count.size();
         ++i) {
        SCOPED_TRACE("jinc_contributor_count_map=" + std::to_string(i));
        expect_matrix_exact(lhs.jinc_products.contributor_count[i],
                            rhs.jinc_products.contributor_count[i]);
    }
    ASSERT_EQ(lhs.jinc_products.formal_support.size(),
              rhs.jinc_products.formal_support.size());
    for (std::size_t i = 0; i < lhs.jinc_products.formal_support.size(); ++i) {
        SCOPED_TRACE("jinc_formal_support_map=" + std::to_string(i));
        expect_matrix_exact(lhs.jinc_products.formal_support[i],
                            rhs.jinc_products.formal_support[i]);
    }
}
...
void seed_destination_state(mapmaking::MapBuffer &map) {
    ...
    seed_matrices(map.jinc_products.denominator_sum_abs);
    seed_matrices(map.jinc_products.contributor_count);
...
TEST(JincParallelOwnership, InconsistentDestinationsFailBeforeMutation) {
    ...
    EXPECT_EQ(
        invoke_invalid(indices, [](auto &omb, auto &) {
            omb.jinc_products.denominator_sum_abs.pop_back();
        }),
        "populate_maps_jinc_parallel ownership-preflight: JINC "
        "conditioning plane inventory does not match signal maps");
    EXPECT_EQ(
        invoke_invalid(indices, [](auto &omb, auto &) {
            omb.jinc_products.contributor_count[4].conservativeResize(
                kRows - 1, kCols);
        }),
        "populate_maps_jinc_parallel ownership-preflight: "
        "omb.jinc_products.contributor_count[4] has dims 6x7; expected 7x7");
    EXPECT_EQ(
        invoke_invalid(indices, [](auto &omb, auto &) {
...
TEST(JincParallelOwnership, ValidUniqueMappingAllowsExtraMapSlots) {
    ...
    EXPECT_TRUE((result.omb.jinc_products.denominator_sum_abs[2].array() ==
                 0.0)
                    .all());
    EXPECT_TRUE((result.omb.jinc_products.contributor_count[2].array() == 0)
                    .all());
}
