...
TEST(timestream_d2_native_measurement,
     line_evidence_rejects_nonfinite_frequencies_before_ordering) {
    const auto fixture = make_fixture();
    for (const double nonfinite : {
             std::numeric_limits<double>::quiet_NaN(),
             std::numeric_limits<double>::infinity(),
             -std::numeric_limits<double>::infinity()}) {
        for (const bool low_field : {true, false}) {
            for (std::size_t position = 0; position < 32; ++position) {
                SCOPED_TRACE(::testing::Message()
                             << "nonfinite=" << nonfinite
                             << " low_field=" << low_field
                             << " position=" << position);
                std::vector<pipeline::D2LineOperatorRecord> records;
                for (std::size_t index = 0; index < 32; ++index) {
                    const double low = 2.0 * (32 - index);
                    records.push_back({"line-" + std::to_string(index),
                                       low, low + 1.0, true, "operator"});
                }
                if (low_field) {
                    records[position].low_frequency_hz = nonfinite;
                } else {
                    records[position].high_frequency_hz = nonfinite;
                }
                EXPECT_THROW(
                    pipeline::D2LineOperatorEvidence::admit(
                        fixture.parent, fixture.route, 0, "nonfinite-frequency",
                        pipeline::D2LineOperatorDisposition::applied,
                        std::move(records)),
                    std::invalid_argument);
            }
    ...
    // Equal low frequencies exercise the comparator's high-frequency branch.
    EXPECT_THROW(
        pipeline::D2LineOperatorEvidence::admit(
            fixture.parent, fixture.route, 0, "nonfinite-tie",
            pipeline::D2LineOperatorDisposition::applied,
            {{"line-a", 1.0, 2.0, true, "operator-a"},
             {"line-nan", 1.0, std::numeric_limits<double>::quiet_NaN(),
              true, "operator-nan"},
             {"line-b", 1.0, 4.0, true, "operator-b"}}),
        std::invalid_argument);
}
