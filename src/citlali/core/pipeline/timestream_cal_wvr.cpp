...
std::shared_ptr<const CalWvrEvidence> CalWvrEvidence::learn(NativeObservationScope scope,
    ...
    std::optional<CalWvrObservationInterval> interval) {
    ...
    if(interval && (!std::isfinite(interval->first_unix_sec) || !std::isfinite(interval->last_unix_sec) ||
       interval->first_unix_sec>interval->last_unix_sec))throw std::invalid_argument("WVR observation interval is malformed");
    out->observation_interval_=interval;
    std::set<std::string> identities;
    for(const auto &r:records) {
        if(r.identity.empty() || !identities.insert(r.identity).second ||
           (r.time_unix_sec && !std::isfinite(*r.time_unix_sec)) ||
           (records.size()>1 && !r.time_unix_sec))
            throw std::invalid_argument("WVR record identity or time is malformed");
    }
    ...
    if(out->single_reading() && !interval)
        throw std::invalid_argument("one WVR reading requires its full original observation interval");
    if(!out->single_reading())for(const auto &r:out->records_)
        if(!std::isfinite(r.valid_first_unix_sec) || !std::isfinite(r.valid_last_unix_sec) ||
           r.valid_first_unix_sec>r.valid_last_unix_sec)
            throw std::invalid_argument("WVR bracket source validity is malformed");
    return out;
}
...
CalWvrSample CalWvrEvidence::at(double t) const {
    ...
    if(single_reading()) {
        const auto &g=groups_.front();const auto &r=records_[g.first];
        out.first_record=g.first;out.last_record=g.first;out.exact_match=r.time_unix_sec && *r.time_unix_sec==t;
        out.observation_constant=true;
        if(g.conflict){out.cause=CalWvrCause::conflicting_duplicate;return out;}
        if(!r.producer_valid){out.cause=CalWvrCause::gap_outside_source_validity;return out;}
        if(!std::isfinite(r.tau225)){out.cause=CalWvrCause::nonfinite;return out;}
        if(r.tau225<0){out.cause=CalWvrCause::negative;return out;}
        if(t<observation_interval_->first_unix_sec || t>observation_interval_->last_unix_sec) {
            out.cause=CalWvrCause::unbracketed;return out;
        }
        out.tau225=r.tau225;out.cause=CalWvrCause::available;return out;
    }
...
CalWvrQuality CalWvrEvidence::quality(double first,double last) const {
    ...
    if(!single_reading())for(const auto &g:groups_) {const auto t=*records_[g.first].time_unix_sec;if(t>first && t<last)times.push_back(t);}
    ...
    q.mean=single_reading()?values.front():q.area/q.duration;
    ...
    q.summary_available=true;q.cause=single_reading()?"single-reading-observation-constant":"complete-source-valid-coverage";
