// Shared application-boundary RTC execution. Scientific owners remain the
// existing typed components; YAML and file checks stop at this boundary.
...
int citlali::cli::run_successor_rtc(const fs::path &input_path, const fs::path &output_path,
  ...
  for(std::size_t i=0;i<arguments.size();++i) pointers[i]=arguments[i].data();
  char **argv=pointers.data(); const int argc=reviewed_selection ? 4 : 3;
  try {
    ...
    if(development) {
      ...
      require(!cfg["speed_support_comparison"] || cfg["speed_support_comparison"].as<std::string>()=="both",
          "successor.unsupported_support_policy: comparison masks are not the accepted RTC support policy");
      require(cfg["reassessment_selection"] &&
          cfg["reassessment_selection"]["intent"].as<std::string>()=="retain-development-candidate",
    ...
    if(argc==4 || recovered_scans){
      ...
      for(bool continuity : (decision && !development ? std::vector<bool>{false,true} : std::vector<bool>{true})){
    ...
    if(development) {
      require(receipt["donor_continuity"]["terminal"]["state"].as<std::string>()=="complete-logical-RTC-terminal",
              "successor.RTC_incomplete: no complete terminal was produced");
      ...
      receipt["completed_through"]="RTC";
      receipt["next_stage"]="CAL";
      receipt["next_stage_state"]="unavailable-contract-implementation-and-input-bindings-required";
      std::cout<<"Timestream Successor: RTC complete; CAL not implemented. No legacy fallback.\n";
    }
