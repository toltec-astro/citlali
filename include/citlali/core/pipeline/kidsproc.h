...
struct KidsDataProc : ConfigMapper<KidsDataProc> {
    ...
    KidsDataProc(config_t config)
        ...
        m_fitter{
            Fitter::Config{
                {"weight_window_type", this->config().get_str(std::tuple{"fitter", "weight_window", "type"})},
                {"weight_window_fwhm", this->config().get_typed<double>(std::tuple{"fitter", "weight_window", "fwhm_Hz"})},
...
auto KidsDataProc::load_fit_report(const RawObs &rawobs) {
    ...
    for (const auto &data_item : rawobs.kidsdata()) {
        ...
        try {
            ...
            throw std::runtime_error(fmt::format("unable to read fitreport file as ECSV {}: {}", filepath,
                                                 e.what()));
            try {
                ...
                throw std::runtime_error(fmt::format("unable to read fitreport file as ASCII {}: {}", filepath,
                                                     e.what()));
            }
        }
    }
...
auto KidsDataProc::load_rawobs(const RawObs &rawobs, const Eigen::Index scan,
                               ...
                               Eigen::DenseBase<DerivedB>& start_indices,
                               Eigen::DenseBase<DerivedB>& end_indices) {
...
auto KidsDataProc::populate_rtc(LoadedType &loaded, const int n_pts,
                                const int n_det, const std::string data_type) {
    // resize data
    ...
    for (std::vector<kids::KidsData<kids::KidsDataKind::RawTimeStream>>::
        ...
        auto block = data.block(0, i, n_rows, n_cols);
