...
struct ParsedMatchedObservationEcsv {
    ...
    // True only when this process revalidated the referenced baseline bytes
    // and receipt. Ordinary consumer admission deliberately trusts the
    // producer-issued embedded reference and therefore records false.
    bool parent_content_revalidated = true;
};
...
inline std::string producer_issued_baseline_descriptor_preimage(
    const VerifiedBaselineReference &reference) {
    std::string preimage;
    detail::add_string(preimage, "encoding", framing_encoding_v1);
    detail::add_string(preimage, "scope", baseline_descriptor_scope_v1);
    detail::add_string(preimage, "schema", baseline_descriptor_schema_v1);
    detail::add_string(preimage, "contract-authority", contract_authority_v1);
    detail::add_string(preimage, "baseline-value-issuer",
                       baseline_value_issuer_v1);
    detail::add_string(preimage, "baseline-schema", reference.artifact.schema);
    detail::add_string(preimage, "baseline-profile", reference.profile);
    detail::add_string(preimage, "baseline-occurrence",
                       reference.artifact.occurrence);
    detail::add_string(preimage, "baseline-semantic-sha256",
                       reference.artifact.semantic_sha256);
    detail::add_string(preimage, "baseline-envelope-sha256",
                       reference.artifact.envelope_sha256);
    detail::add_string(preimage, "baseline-transport-scope",
                       reference.transport_scope);
    detail::add_string(preimage, "baseline-transport-sha256",
                       reference.transport_sha256);
    detail::add_uint64(preimage, "baseline-byte-count", reference.byte_count);
    detail::add_string(preimage, "receipt-sha256", reference.receipt_sha256);
    detail::add_uint64(preimage, "receipt-byte-count",
                       reference.receipt_byte_count);
    return preimage;
}

inline std::string producer_issued_baseline_descriptor_sha256(
    const VerifiedBaselineReference &reference) {
    return "sha256:" + citlali::utils::sha256(
        producer_issued_baseline_descriptor_preimage(reference));
}

inline void validate_producer_issued_baseline_reference(
    const VerifiedBaselineReference &reference) {
    detail::require_artifact_identity(reference.artifact);
    baseline::detail::require_text("verified baseline profile",
                                   reference.profile);
    if (reference.artifact.schema != baseline::schema_version_v1 ||
        reference.profile != baseline::baseline_profile_v1 ||
        !baseline::is_sha256_reference(reference.descriptor_sha256) ||
        reference.transport_scope != baseline::byte_transport_scope_v1 ||
        !baseline::is_sha256_reference(reference.transport_sha256) ||
        reference.byte_count == 0 ||
        !baseline::is_sha256_reference(reference.receipt_sha256) ||
        reference.receipt_byte_count == 0 ||
        reference.descriptor_sha256 !=
            producer_issued_baseline_descriptor_sha256(reference)) {
        throw baseline::ContractError(
            "producer-issued baseline reference is not a complete canonical v1 identity");
    }
}
...
inline OutputFieldContract target_output_field_contract_v1(
    const TypedField &source) {
    return {{source.name,
             source.type,
             source.unit,
             source.nullable,
...
inline OutputFieldContract baseline_output_field_contract_v1(
    const baseline::RegisteredField &source) {
    return {{source.name,
             source.type,
             source.unit,
             true,
...
inline std::map<std::string, OutputFieldContract>
validate_producer_issued_output_field_contracts_v1(
    const MatchedOutput &document, const TargetManifest &target) {
    validate(target);
    std::map<std::string, OutputFieldContract> declared;
    for (const auto &contract : document.registered_fields) {
        ...
        if (!declared.emplace(contract.field.name, contract).second) {
            throw baseline::ContractError(
                "duplicate matched-output field contract");
        }
    }

    std::map<std::string, OutputFieldContract> required;
    std::map<std::string, OutputFieldContract> allowed;
    std::set<std::string> reserved_target_names;
    for (const auto &field : canonical_target_fields_v1()) {
        reserved_target_names.insert(field.name);
    }
    for (const auto &field : target.registered_fields) {
        const auto contract = target_output_field_contract_v1(field);
        required.emplace(field.name, contract);
        allowed.emplace(field.name, contract);
    }
    const auto &registry = baseline::canonical_field_registry_v1();
    for (const auto &field : registry.required_baseline_fields) {
        if (reserved_target_names.contains(field.name)) {
            continue;
        }
        const auto contract = baseline_output_field_contract_v1(field);
        required.emplace(field.name, contract);
        allowed.emplace(field.name, contract);
    }
    for (const auto &field : registry.optional_extensions) {
        ...
        allowed.emplace(field.name, baseline_output_field_contract_v1(field));
    }

    for (const auto &[name, expected] : required) {
        const auto found = declared.find(name);
        if (found == declared.end() || found->second != expected) {
            throw baseline::ContractError(
                "producer-issued matched output omits or changes a required closed v1 field contract: " +
                name);
        }
    }
    for (const auto &[name, contract] : declared) {
        const auto expected = allowed.find(name);
        if (expected == allowed.end() || expected->second != contract) {
            throw baseline::ContractError(
                "producer-issued matched output declares a field or operation outside the closed Citlali v1 registries: " +
                name);
        }
    }
    return declared;
...
inline void validate_producer_issued(
    const MatchRelation &document, const TargetManifest &target) {
    validate(target);
    validate_producer_issued_baseline_reference(document.baseline_parent);
    if (document.schema != relation_schema_v1 ||
        document.contract_authority != contract_authority_v1 ||
        document.observation_value_issuer != observation_value_issuer_v1 ||
        document.mapping_domain != mapping_domain_v1 ||
        document.target_parent != artifact_identity(target)) {
        throw baseline::ContractError(
            "producer-issued relation schema, authority, mapping domain, or target parent is invalid");
    }
    detail::require_envelope(document.envelope);
    if (target.envelope.occurrence ==
            document.baseline_parent.artifact.occurrence ||
        document.envelope.occurrence == target.envelope.occurrence ||
        document.envelope.occurrence ==
            document.baseline_parent.artifact.occurrence) {
        throw baseline::ContractError(
            "producer-issued target, relation, and baseline-reference occurrences must be distinct");
    }
    baseline::detail::require_text("matcher-run occurrence",
                                   document.matcher.matcher_run_occurrence);
    baseline::detail::require_text("matcher implementation revision",
    ...
    if (document.seed_source_sequence.empty()) {
        throw baseline::ContractError(
            "producer-issued relation requires a nonempty embedded seed inventory");
    }
    const std::set<std::int64_t> seed_keys(
        document.seed_source_sequence.begin(),
        document.seed_source_sequence.end());
    detail::require_permutation(document.seed_source_sequence, seed_keys,
                                "seed source sequence");

    const auto target_identity = artifact_identity(target);
    const auto &seed_identity = document.baseline_parent.artifact;
    std::map<std::int64_t, MatchPair> pairs;
    std::map<std::int64_t, std::set<std::int64_t>> target_pairs;
    std::map<std::int64_t, std::set<std::int64_t>> seed_pairs;
    std::set<std::pair<std::int64_t, std::int64_t>> endpoints;
    ...
    for (const auto &pair : document.pairs) {
        ...
        if (!relation_row_keys.insert(pair.pair_key).second ||
            !pairs.emplace(pair.pair_key, pair).second ||
            pair.target != detail::make_row_reference(
                               target_identity, pair.target.local_key) ||
            pair.seed != detail::make_row_reference(
                             seed_identity, pair.seed.local_key) ||
            !target_keys.contains(pair.target.local_key) ||
            !seed_keys.contains(pair.seed.local_key) ||
            !std::isfinite(pair.separation_hz) || pair.separation_hz < 0.0 ||
            !endpoints.emplace(pair.target.local_key, pair.seed.local_key)
                 .second) {
            throw baseline::ContractError(
                "producer-issued match pair key/endpoints/separation is invalid or duplicate");
        }
        target_pairs[pair.target.local_key].insert(pair.pair_key);
        seed_pairs[pair.seed.local_key].insert(pair.pair_key);
    }
    ...
    for (const auto &evidence : document.network_evidence) {
        if (!target_networks.contains(evidence.network) ||
            ...
            evidence.quality_factor_authority_reference !=
                kmp_model_params_authority_reference_v1) {
            throw baseline::ContractError(
                "producer-issued relation network evidence is invalid or incomplete");
        }
    }
    if (evidence_networks != target_networks) {
        throw baseline::ContractError(
            "producer-issued relation requires evidence for every target network");
    }
        ...
        for (const auto &disposition : dispositions) {
            ...
            if (!relation_row_keys.insert(disposition.disposition_key).second ||
                ...
                disposition.endpoint !=
                    detail::make_row_reference(identity, key)) {
                throw baseline::ContractError(
                    "producer-issued relation disposition endpoint is foreign or duplicate");
            }
            detail::require_sorted_unique_pair_keys(disposition.pair_keys,
                                                    "disposition pair keys");
            const auto found = expected_pairs.find(key);
            ...
            if (actual != expected ||
                ...
                (actual.empty() && !target_side &&
                 disposition.state != EndpointDispositionState::unused)) {
                throw baseline::ContractError(
                    "producer-issued relation disposition is not reciprocal or complete");
            }
        }
        if (seen != expected_keys) {
            throw baseline::ContractError(
                "producer-issued relation dispositions do not cover every embedded endpoint");
        }
    };
    validate_dispositions(document.target_dispositions, target_keys,
                          target_pairs, target_identity, true);
...
inline std::string relation_semantic_preimage_after_validation(
...
inline std::string relation_semantic_preimage(
    ...
    return relation_semantic_preimage_after_validation(document);
}

inline std::string producer_issued_relation_semantic_preimage(
    const MatchRelation &document, const TargetManifest &target) {
    validate_producer_issued(document, target);
    return relation_semantic_preimage_after_validation(document);
}
...
inline std::string producer_issued_relation_semantic_sha256(
    const MatchRelation &document, const TargetManifest &target) {
    return "sha256:" + citlali::utils::sha256(
        producer_issued_relation_semantic_preimage(document, target));
}

inline std::string producer_issued_relation_envelope_preimage(
    const MatchRelation &document, const TargetManifest &target) {
    validate_producer_issued(document, target);
    std::string preimage;
    detail::add_envelope(
        preimage, document.schema, relation_envelope_scope_v1,
        producer_issued_relation_semantic_sha256(document, target),
        document.envelope);
    return preimage;
}

inline std::string producer_issued_relation_envelope_sha256(
    const MatchRelation &document, const TargetManifest &target) {
    return "sha256:" + citlali::utils::sha256(
        producer_issued_relation_envelope_preimage(document, target));
}

inline baseline::Digests compute_producer_issued_digests(
    const MatchRelation &document, const TargetManifest &target) {
    return {producer_issued_relation_semantic_sha256(document, target),
            producer_issued_relation_envelope_sha256(document, target)};
}

inline ArtifactIdentity producer_issued_artifact_identity(
    const MatchRelation &document, const TargetManifest &target) {
    const auto digests = compute_producer_issued_digests(document, target);
    return {document.schema, document.envelope.occurrence,
            digests.semantic_sha256, digests.envelope_sha256};
}
...
inline void validate_producer_issued(
    ...
    validate_producer_issued(relation, target);
    if (document.schema != matched_output_schema_v1 ||
        document.contract_authority != contract_authority_v1 ||
        document.observation_value_issuer != observation_value_issuer_v1 ||
        document.transformation_registry != transformation_registry_v1 ||
        document.baseline_parent != relation.baseline_parent ||
        document.target_parent != artifact_identity(target) ||
        document.relation_parent !=
            producer_issued_artifact_identity(relation, target)) {
        throw baseline::ContractError(
            "producer-issued matched output schema, authority, registry, or embedded parent is invalid");
    }
    detail::require_envelope(document.envelope);
    if (document.envelope.occurrence ==
            document.baseline_parent.artifact.occurrence ||
        ...
        document.envelope.occurrence == relation.envelope.occurrence) {
        throw baseline::ContractError(
            "producer-issued matched-output occurrence must be distinct from every embedded parent occurrence");
    }

    const auto output_fields =
        validate_producer_issued_output_field_contracts_v1(document, target);
    std::map<std::int64_t, const TargetRow *> target_rows;
    for (const auto &row : target.rows) {
        target_rows.emplace(row.row_key, &row);
    }
    ...
    for (const auto &row : document.rows) {
        ...
        if (!output_keys.insert(row.uid).second ||
            row.target != detail::make_row_reference(
                              target_identity, row.target.local_key) ||
            !covered_targets.insert(row.target.local_key).second) {
            throw baseline::ContractError(
                "producer-issued matched-output uid or target reference is duplicate/foreign");
        }
        const auto target_row_it = target_rows.find(row.target.local_key);
        if (target_row_it == target_rows.end()) {
            throw baseline::ContractError(
                "producer-issued matched-output row references an absent target");
        }
        const auto &target_row = *target_row_it->second;
        if (row.target_input_key != target_row.input_key ||
            row.array != target_row.array || row.network != target_row.network ||
            ...
            canonical_binary64_payload(row.tone_frequency_hz) !=
                canonical_binary64_payload(
                    target_row.output_tone_frequency_hz)) {
            throw baseline::ContractError(
                "producer-issued matched-output structural/raw values differ from the target");
        }
        const auto disposition =
            target_dispositions.find(row.target.local_key);
        if (disposition == target_dispositions.end() ||
            row.relation_pair_keys != disposition->second.pair_keys) {
            throw baseline::ContractError(
                "producer-issued matched-output relation pair set is incomplete or reordered");
        }

        if (row.fields.size() != output_fields.size() ||
            row.transformations.size() != output_fields.size()) {
            throw baseline::ContractError(
                "producer-issued matched-output row lacks exact fields/transformations");
        }
        std::map<std::string, FieldTransformation> transformations;
        for (const auto &transformation : row.transformations) {
            if (!transformations
                     ...
                     .second) {
                throw baseline::ContractError(
                    "duplicate producer-issued matched-output field transformation");
            }
        }
        for (const auto &[name, contract] : output_fields) {
            const auto output_value = row.fields.find(name);
            ...
            if (output_value == row.fields.end() ||
                ...
                !detail::values_equal(transformation->second.after,
                                      ...
                                      contract.field.type)) {
                throw baseline::ContractError(
                    "producer-issued matched-output transformation is untyped or unauthorized: " +
                    name);
            }
            const auto &change = transformation->second;
            baseline::detail::require_text("transformation authority",
                    ...
                    *change.source_row != row.target ||
                    ...
                    change.provenance_reference !=
                        "target-kmp-source:" +
                            ...
                            *contract.field.source_column) {
                    throw baseline::ContractError(
                        "producer-issued preserve-target transformation does not retain its exact target KMP value/source/authority");
                }
                continue;
            }
            if (contract.authorized_operation !=
                ...
                !std::holds_alternative<baseline::NullValue>(change.before)) {
                throw baseline::ContractError(
                    "producer-issued matched-output v1 admits no caller-selected transformation operation");
            }
            if (matched) {
                if (change.value_source !=
                        TransformationValueSource::baseline_seed_row ||
                    ...
                    !std::binary_search(row.relation_pair_keys.begin(),
                                        ...
                                        *change.source_pair_key)) {
                    throw baseline::ContractError(
                        "producer-issued baseline transformation lacks an exact relation-pair source");
                }
                const auto pair = pairs.find(*change.source_pair_key);
                if (pair == pairs.end() || pair->second.target != row.target ||
                    pair->second.seed != *change.source_row ||
                    ...
                    change.provenance_reference !=
                        "relation-pair:" +
                            std::to_string(*change.source_pair_key)) {
                    throw baseline::ContractError(
                        "producer-issued baseline transformation pair/source/authority is inconsistent");
                }
            } else if (change.value_source !=
                           TransformationValueSource::canonical_null ||
                       change.source_pair_key || change.source_row ||
                       ...
                       change.provenance_reference !=
                           "target-unmatched:no-fabricated-seed") {
                throw baseline::ContractError(
                    "producer-issued unmatched transformation is not the registered typed missing value");
            }
        }
    }
    if (covered_targets.size() != target.rows.size()) {
        throw baseline::ContractError(
            "producer-issued matched output does not contain exactly one row per target");
    }
    detail::require_permutation(document.output_presentation_sequence,
                                output_keys,
                                "matched-output presentation sequence");
...
inline std::string matched_output_semantic_preimage_after_validation(
    const MatchedOutput &document) {
    std::string preimage;
    detail::add_string(preimage, "encoding", framing_encoding_v1);
...
inline std::string matched_output_semantic_preimage(
    ...
    return matched_output_semantic_preimage_after_validation(document);
}

inline std::string producer_issued_matched_output_semantic_preimage(
    const MatchedOutput &document, const TargetManifest &target,
    const MatchRelation &relation) {
    validate_producer_issued(document, target, relation);
    return matched_output_semantic_preimage_after_validation(document);
}
...
inline std::string producer_issued_matched_output_semantic_sha256(
    const MatchedOutput &document, const TargetManifest &target,
    const MatchRelation &relation) {
    return "sha256:" + citlali::utils::sha256(
        producer_issued_matched_output_semantic_preimage(document, target,
                                                         relation));
}

inline std::string producer_issued_matched_output_envelope_preimage(
    const MatchedOutput &document, const TargetManifest &target,
    const MatchRelation &relation) {
    validate_producer_issued(document, target, relation);
    std::string preimage;
    detail::add_envelope(
        preimage, document.schema, matched_output_envelope_scope_v1,
        producer_issued_matched_output_semantic_sha256(document, target,
                                                       relation),
        document.envelope);
    return preimage;
}

inline std::string producer_issued_matched_output_envelope_sha256(
    const MatchedOutput &document, const TargetManifest &target,
    const MatchRelation &relation) {
    return "sha256:" + citlali::utils::sha256(
        producer_issued_matched_output_envelope_preimage(document, target,
                                                         relation));
}
...
inline baseline::Digests compute_producer_issued_digests(
    const MatchedOutput &document, const TargetManifest &target,
    const MatchRelation &relation) {
    return {producer_issued_matched_output_semantic_sha256(document, target,
                                                           relation),
            producer_issued_matched_output_envelope_sha256(document, target,
                                                           relation)};
}
...
inline ArtifactIdentity producer_issued_artifact_identity(
    ...
    const MatchRelation &relation) {
    const auto digests =
        compute_producer_issued_digests(document, target, relation);
    return {document.schema, document.envelope.occurrence,
            digests.semantic_sha256, digests.envelope_sha256};
}
...
inline void emit_relation_after_validation(std::ostringstream &stream,
                                           const MatchRelation &document) {
...
inline void emit_relation(std::ostringstream &stream,
    ...
    emit_relation_after_validation(stream, document);
}

inline void emit_producer_issued_relation(
    std::ostringstream &stream, const MatchRelation &document,
    const TargetManifest &target) {
    validate_producer_issued(document, target);
    emit_relation_after_validation(stream, document);
}

inline MatchRelation parse_relation_fields(MetadataReader &reader) {
...
inline MatchRelation parse_relation(
    ...
    const TargetManifest &target) {
    auto document = parse_relation_fields(reader);
    validate(document, descriptor, target);
    return document;
}

inline MatchRelation parse_producer_issued_relation(
    MetadataReader &reader, const TargetManifest &target) {
    auto document = parse_relation_fields(reader);
    validate_producer_issued(document, target);
    return document;
}
...
inline SerializedMatchedObservationEcsv
serialize_matched_observation_ecsv_after_validation(
    const MatchedOutput &document, const TargetManifest &target,
    const MatchRelation &relation, const baseline::Digests &digests,
    const baseline::Digests &relation_digests) {
    ...
    wire::emit_relation_after_validation(stream, relation);
...
inline SerializedMatchedObservationEcsv serialize_matched_observation_ecsv(
    ...
    return serialize_matched_observation_ecsv_after_validation(
        document, target, relation, digests, relation_digests);
}

inline SerializedMatchedObservationEcsv
serialize_producer_issued_matched_observation_ecsv(
    const MatchedOutput &document, const TargetManifest &target,
    const MatchRelation &relation) {
    validate_producer_issued(document, target, relation);
    const auto digests =
        compute_producer_issued_digests(document, target, relation);
    const auto relation_digests =
        compute_producer_issued_digests(relation, target);
    return serialize_matched_observation_ecsv_after_validation(
        document, target, relation, digests, relation_digests);
}

inline ParsedMatchedObservationEcsv parse_matched_observation_ecsv_impl(
    std::string_view bytes,
    const VerifiedBaselineDescriptor *baseline_descriptor) {
    namespace wire = observation_ecsv_detail;
    if (bytes.empty() || bytes.back() != '\n') {
    ...
    auto relation = baseline_descriptor == nullptr
        ? wire::parse_producer_issued_relation(reader, target)
        : wire::parse_relation(reader, *baseline_descriptor, target);
    const auto relation_digests = baseline_descriptor == nullptr
        ? compute_producer_issued_digests(relation, target)
        : compute_digests(relation, *baseline_descriptor, target);
    ...
    baseline::Digests computed;
    if (baseline_descriptor == nullptr) {
        validate_producer_issued(document, target, relation);
        computed = compute_producer_issued_digests(document, target, relation);
    } else {
        validate(document, *baseline_descriptor, target, relation);
        computed = compute_digests(document, *baseline_descriptor, target,
                                   relation);
    }
    ...
    const auto canonical = baseline_descriptor == nullptr
        ? serialize_producer_issued_matched_observation_ecsv(
              document, target, relation)
        : serialize_matched_observation_ecsv(
              document, *baseline_descriptor, target, relation);
    ...
    return {std::move(target), std::move(relation), std::move(document),
            ...
            baseline_descriptor != nullptr};
}

inline ParsedMatchedObservationEcsv parse_matched_observation_ecsv(
    std::string_view bytes,
    const VerifiedBaselineDescriptor &baseline_descriptor) {
    return parse_matched_observation_ecsv_impl(bytes, &baseline_descriptor);
}

inline ParsedMatchedObservationEcsv
parse_issued_matched_observation_ecsv(std::string_view bytes) {
    return parse_matched_observation_ecsv_impl(bytes, nullptr);
}
...
inline ParsedMatchedObservationEcsv
parse_issued_matched_observation_ecsv_with_receipt(
    std::string_view bytes, std::string_view receipt_bytes) {
    namespace publication = canonical_artifact_publication;
    const auto receipt = publication::parse_canonical_receipt(
        receipt_bytes, publication::receipt_schema_v1,
        matched_output_byte_transport_scope_v1);
    publication::validate_receipt_binding(bytes, receipt);
    auto parsed = parse_issued_matched_observation_ecsv(bytes);
    if (receipt.envelope_sha256 !=
        parsed.declared_digests.envelope_sha256) {
        throw baseline::ContractError(
            "producer-issued matched observation receipt binds a foreign envelope");
    }
    if (parsed.parent_content_revalidated) {
        throw baseline::ContractError(
            "consumer admission incorrectly claims baseline parent-content replay");
    }
    return parsed;
}
