...
bool prepare_initial_observation(
    ...
    return prepare_initial_observation_setup<IsBeammap>(
        todproc, rawobs, rawobs_kids_meta, map_extents, map_coords,
...
bool prepare_initial_observations(
    ...
    if constexpr (has_astrometry_plan_v<
                      decltype(todproc.engine())>) {
        astrometry_plan(todproc.engine()).reset(co.n_inputs());
    }
