...
template <class MapPixelOutlier, class Candidates, class MapBuffer,
          class ReductionLearning>
void emit_mapdiag_outlier_records(
...
template <class DetectorPenalty, class Candidates, class ReductionLearning,
          class Arrays, class Logger>
void emit_mapdiag_detector_penalty_learning(
    Candidates &candidates, Eigen::Index map_index,
    Eigen::Index write_map_index, const Arrays &arrays,
    const std::string &obsnum, const std::string &record_producer,
    const std::string &stage_name, const std::string &evidence_view,
    int fruit_iter, int fill_int, ReductionLearning &reduction_learning,
    const Logger &logger) {
    ...
    for (std::size_t ci = 0; ci < n_emitted_candidates; ++ci) {
        update_mapdiag_detector_dominance(
            dominance, mapdiag_emitted_candidate(candidates, ci), fill_int);
    }
    ...
    for (const auto &entry : dominance) {
        ...
        logger->info(
            "mapdiag learned scan-local detector exclusion candidate stage={} iter={} map={} evidence_view={} uid={} scan={} outlier_pixels={} max_abs_value={:.4g} max_abs_leave_one_out_z={:.4g}",
            stage_name, fruit_iter, map_index, evidence_view, entry.uid,
