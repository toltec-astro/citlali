...
using ShapeType = Eigen::Matrix<IndexType, 2, 1>;
...
private:
    ...
    std::vector<int> m_kidsdata_indices{};
    std::optional<int> m_teldata_index{std::nullopt};
    std::optional<int> m_hwpdata_index{std::nullopt};

    void collect_data_items();
    std::vector<CalItem> m_cal_items{};
    std::optional<int> m_apt_index{std::nullopt};
    std::optional<int> m_phot_cal_index{std::nullopt};
    std::optional<int> m_astro_cal_index{std::nullopt};
...
void RawObs::collect_data_items() {
    ...
    for (int i = 0; i < m_data_items.size(); ++i) {
...
void RawObs::collect_cal_items() {
    ...
    for (int i = 0; i < m_cal_items.size(); ++i) {
...
struct SeqIOCoordinator : ConfigMapper<SeqIOCoordinator> {
    ...
    using IndexType = predefs::IndexType;
    using ShapeType = predefs::ShapeType;
    using InputType = RawObs;
    using PayloadType = RawObs::DataItem;
    ...
    // io_buffer
    using PayloadsBufferDataType = predefs::DataType;
...
private:
    std::vector<InputType> m_inputs{}; // stores all the inputs
