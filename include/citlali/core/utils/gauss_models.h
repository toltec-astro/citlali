...
struct CeresAutoDiffFitter: Fitter<Model> {
    ...
    std::shared_ptr<Problem> createProblem(double* params) {
        ...
        if (logger) {
            logger->trace("ceres_fit checkpoint: createProblem internals start param_ptr={} model_param_size={}",
                         static_cast<const void*>(params), this->model()->params.size());
        }
        std::shared_ptr<Problem> problem = std::make_shared<Problem>();
        if (logger) {
            logger->trace("ceres_fit checkpoint: createProblem Problem() constructed ptr={}",
                         static_cast<const void*>(problem.get()));
        }
        problem->AddParameterBlock(params, this->model()->params.size());
        if (logger) {
            logger->trace("ceres_fit checkpoint: createProblem AddParameterBlock done size={}",
