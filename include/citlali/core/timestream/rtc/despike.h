...
void Despiker::despike(Eigen::DenseBase<DerivedA> &scans,
    ...
    for (Eigen::Index det = 0; det < ndet; det++) {
        ...
        auto cutoff = min_spike_sigma * engine_utils::calc_std_dev(delta);

        // mean subtracted delta array
        Eigen::VectorXd diff = abs(delta.array() - delta.mean());

        // run the spike finder,
        spike_finder(flags.col(det), delta, diff, n_spikes, cutoff);
        ...
        n_spikes = (flags.col(det).segment(1,npts - 2).array() == 0).count();
        ...
        if (n_spikes > 0) {
            // recount spikes
            n_spikes = (flags.col(det).head(npts - 1).array() == 0).count();
            ...
            auto [win_index_0, win_index_1, win_size] =
                make_window(spike_loc, n_spikes, npts);
            ...
            engine_utils::smooth_boxcar(sub_vals, smoothed_sub_vals, 10);
...
void Despiker::replace_spikes(Eigen::DenseBase<DerivedA> &scans, Eigen::DenseBase<DerivedB> &flags,
    ...
    n_flagged = ndet - has_spikes.template cast<int>().sum();

    SPDLOG_INFO("has spikes {}", has_spikes);
        ...
        if (!has_spikes(det)) {
            ...
            for (Eigen::Index j = 0; j < n_flagged_regions; j++) {
                ...
                if (use_all_det) {
                    ...
                    det_count = has_spikes.template cast<int>().sum();
                }

                Eigen::MatrixXd detm(nflags, det_count);
                detm.setConstant(-99);
                ...
                for (Eigen::Index ii = 0; ii < ndet; ii++) {
                    if (has_spikes(ii) || use_all_det) {
                        SPDLOG_INFO("c {}", c);
                        detm.col(c) =
                            scans.block(si_flags(j), ii, nflags, 1);
                ...
                for (Eigen::Index ii = 0; ii < det_count; ii++) {
                    ...
                    SPDLOG_INFO("tmp_vec {}",tmp_vec);
                    double tmp_mean = tmp_vec.mean();
                    SPDLOG_INFO("tmp_mean {}",tmp_mean);
                    ...
                    SPDLOG_INFO("std_dev_ff 1{}",std_dev_ff);
                ...
                Eigen::VectorXd error =
                    Eigen::VectorXd::Zero(nflags).unaryExpr([&](double dummy){return rands(eng);});

                SPDLOG_INFO("error {}", error);

                // the noiseless fake data is then the sky model plus the
                // flagged detectors linear offset
                Eigen::VectorXd fake = sky_model.array() * responsivity(det) + lin_offset.array() + error.array();
