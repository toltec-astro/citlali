...
void Engine::write_mapdiag(map_buffer_t &mb, std::string dir_name) {
    ...
    for (Eigen::Index i = 0; i < n_maps; ++i) {
        ...
        const auto labels = citlali::pipeline::make_mapdiag_map_labels(
            ...
            get_map_name(i));
        array_names[idx] = labels.array_name;
        stokes_names[idx] = labels.stokes_name;
        ...
        if (citlali::pipeline::mapdiag_has_coverage_map(mb->coverage, i)) {
            citlali::pipeline::assign_mapdiag_coverage_stats(
                mb->coverage[i], core_mask, fill_double, coverage_sum[idx],
                coverage_max[idx], coverage_median_core[idx]);
        }
        ...
        if (citlali::pipeline::mapdiag_has_matrix_samples(mb->signal[i]) &&
            ...
            if (citlali::pipeline::mapdiag_has_noise_realizations(
                ...
                noise_tail_frac_abs3[idx] = mapdiag_stats.median(noise_samples.tail_abs);
                noise_tail_frac_pos3[idx] = mapdiag_stats.median(noise_samples.tail_pos);
                noise_tail_frac_neg3[idx] = mapdiag_stats.median(noise_samples.tail_neg);
                noise_tail_excess_abs3[idx] = mapdiag_stats.median(noise_samples.excess_abs);
                noise_tail_excess_pos3[idx] = mapdiag_stats.median(noise_samples.excess_pos);
                noise_tail_excess_neg3[idx] = mapdiag_stats.median(noise_samples.excess_neg);
                noise_sig2noise_skew[idx] = mapdiag_stats.median(noise_samples.skew);
            }
        }
