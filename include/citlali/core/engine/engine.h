...
void Engine::write_mapdiag(map_buffer_t &mb, std::string dir_name) {
    ...
    std::vector<double> noise_tail_excess_abs3(mapdiag_context.n_maps, fill_double);
    ...
    auto make_mapdiag_off_source_core_mask =
        [&](const auto &core_mask,
            ...
            const double protect_radius =
                mapdiag_current_source_protect_radius_arcsec();
            return citlali::pipeline::mapdiag_off_source_core_mask(
                core_mask, source_distance_context, protect_radius);
        };
    auto make_mapdiag_current_off_source_core_mask =
        [&](const auto &core_mask,
            const auto &source_distance_context) {
            return make_mapdiag_off_source_core_mask(
                core_mask, source_distance_context);
        };
    auto collect_mapdiag_off_source_values =
    ...
    auto collect_mapdiag_current_off_source_values =
        [&](const Eigen::MatrixXd &sig2noise,
            const Eigen::ArrayXXd &off_source_core_mask) {
            return collect_mapdiag_off_source_values(
                sig2noise, off_source_core_mask);
        };
    ...
    auto assign_mapdiag_current_candidate_leave_one_out_z =
        [&](double value, double weight, double leave_one_out_value,
            map_pixel_candidate_t &candidate) {
            assign_mapdiag_current_leave_one_out_z(
                value, weight, leave_one_out_value, candidate);
        };
    ...
    auto log_mapdiag_current_detector_penalty =
        [&](Eigen::Index map_i, const detector_dominance_t &entry) {
            logger->info(
                ...
                mapdiag_current_dominance_display_scan_index(entry),
    ...
    auto make_mapdiag_current_noise_tail_samples =
        [&]() {
            return citlali::pipeline::make_mapdiag_noise_tail_samples(mb);
        };
    auto make_mapdiag_current_valid_core_noise_mask =
        [](const Eigen::ArrayXXd &core_mask) {
            return citlali::pipeline::mapdiag_valid_core_noise_mask(
                core_mask);
        };
    auto mapdiag_current_valid_core_noise_count =
        [](const auto &valid_core) {
            return citlali::pipeline::mapdiag_valid_core_noise_count(
                valid_core);
        };
                            ...
                            for (Eigen::Index c = 0;
                                ...
                                const double z =
                                    mapdiag_current_candidate_robust_z(
                                        sn, robust_stats);
                                if (!mapdiag_current_candidate_passes_min_abs_z(
                                        z)) {
                                    continue;
                                }
                                ...
                                if (have_contrib) {
                                    ...
                                    const double contrib_weight =
                                        mapdiag_current_contributor_weight(
                                            map_st, r, c);
