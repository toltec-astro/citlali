    ...
    if constexpr (prod_t == engine_utils::toltecIO::ptc_timestream) {
        ...
        if (ptcproc.cleaner.adaptive_selector.enabled) {
            add_ptc_stream_network_block(
                ...
                citlali::pipeline::ptcdiag_adaptive_pca_int_vars(),
                citlali::pipeline::ptcdiag_adaptive_pca_double_vars());
        }
    }
...
void Engine::add_phdu(fits_io_type &fits_io, map_buffer_t &mb, Eigen::Index i) {
    ...
    add_double_key("CONFIG.WEIGHT.HYBRID_MIN", ptcproc.hybrid_correction_min_factor,
