...
struct TimeOrderedDataProc : ConfigMapper<TimeOrderedDataProc<EngineType>> {
    ...
    auto align_timestreams(const RawObs &rawobs) {
        ...
        for (const RawObs::DataItem &data_item : rawobs.kidsdata()) {
            try {
                ...
                SPDLOG_INFO("pps {} nw_ts.back().size() {}", pps_msec, nw_ts.back().size());
        ...
        SPDLOG_INFO("xi {}", nw_ts[max_t0_i]);
        SPDLOG_INFO("teltime {}", engine().telescope.tel_data["TelTime"]);
        SPDLOG_INFO("min {}", (engine().telescope.tel_data["TelTime"] - nw_ts[max_t0_i]).minCoeff());
        SPDLOG_INFO("max {}", (engine().telescope.tel_data["TelTime"] - nw_ts[max_t0_i]).maxCoeff());
        ...
        for (const auto &tel_it : engine().telescope.tel_data) {
            if (tel_it.first !="TelTime") {
                ...
                SPDLOG_INFO("before {} {}",tel_it.first, yd);
                ...
                SPDLOG_INFO("after {} {}",tel_it.first, yi);

                engine().telescope.tel_data[tel_it.first] = yi;
            }
        }
    ...
    auto calc_map_num() {
        ...
        SPDLOG_INFO("engine().n_maps {} {} {}", engine().n_maps, engine().redu_type, engine().map_grouping);
    }

    // determine the map dimensions and allocate the coadded map buffer
    auto allocate_cmb(std::vector<map_extent_t> &map_extents, std::vector<map_coord_t> &map_coords) {
    ...
    template <class map_buffer>
    auto allocate_nmb(map_buffer &mb) {
    ...
    auto allocate_omb(map_extent_t &map_extent, map_coord_t &map_coord) {
    ...
    auto calc_map_size(std::vector<map_extent_t> &map_extents, std::vector<map_coord_t> &map_coords) {
