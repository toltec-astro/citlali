        ...
        if ((in.flags.data.col(i).array()==0).any()) {
            ...
            for (Eigen::Index j=0; j<n_pts; ++j) {
                ...
                if (!in.flags.data(j,i)) {
                    ...
                    if (run_polarization && apt["fg"](det_index)!=-1) {
                        cos_2angle = cos(2.*in.angle.data(j,i));
                        sin_2angle = sin(2.*in.angle.data(j,i));
                        logger->info("cos_2angle {} sin_2angle {} {}",cos_2angle,sin_2angle,apt["fg"](det_index));
                    }
                    ...
                    if (run_omb) {
                        ...
                        if ((omb_ir >= 0) && (omb_ir < omb.n_rows) && (omb_ic >= 0) && (omb_ic < omb.n_cols)) {
                            ...
                            if (run_polarization && apt["fg"](det_index)!=-1) {
                                ...
                                omb.weight[q_index](omb_ir,omb_ic) += in.weights.data(i);
                                omb.weight[u_index](omb_ir,omb_ic) += in.weights.data(i);

                                // populate coverage map
                                if (run_coverage) {
                                    omb.coverage[q_index](omb_ir,omb_ic) += 1./d_fsmp;
                                    omb.coverage[u_index](omb_ir,omb_ic) += 1./d_fsmp;
                                }
