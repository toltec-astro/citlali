...
constexpr std::string_view schema = "citlali-wp7-rtc-filter-fixture-census-v3";
...
std::string usage() {
  return "Usage: citlali_wp7_rtc_filter_fixture_census\n"
...
Arguments parse_arguments(int argc, char **argv) {
  ...
  auto next = [&](int &index, std::string_view option) {
    require(index + 1 < argc, std::string(option) + " requires a value");
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (option == "--help" || option == "-h") {
      ...
      result.auxiliary_inputs.emplace_back(role, next(index, option));
    } else {
  ...
  require(result.source_revision.size() == 40 &&
              std::all_of(
                  result.source_revision.begin(), result.source_revision.end(),
                  [](unsigned char ch) {
...
std::string read_netcdf_text(netCDF::NcFile &file, const std::string &name) {
  const auto variable = file.getVar(name);
...
std::string read_units(const netCDF::NcVar &variable, const std::string &name) {
...
Eigen::VectorXd read_vector(netCDF::NcFile &file, const std::string &name,
...
Eigen::VectorXi read_integer_vector(netCDF::NcFile &file,
  ...
  require(!variable.isNull() && variable.getDimCount() == 1 &&
              variable.getDim(0).getSize() == expected_count,
          name + " has an unexpected shape");
  Eigen::VectorXi result(static_cast<Eigen::Index>(expected_count));
  variable.getVar(result.data());
  return result;
}

double read_radian_scalar(netCDF::NcFile &file, const std::string &name) {
  const auto variable = file.getVar(name);
  require(!variable.isNull(), "missing NetCDF scalar " + name);
  require(read_units(variable, name) == "radian",
          name + " has unexpected units");
  double result = 0.0;
  variable.getVar(&result);
  return result;
...
std::string_view
route_profile_token(pipeline::AstScanMotionRouteProfile profile) {
  switch (profile) {
  case pipeline::AstScanMotionRouteProfile::science_lissajous:
    return "science-lissajous";
  case pipeline::AstScanMotionRouteProfile::oof_lissajous:
    return "oof-lissajous";
  case pipeline::AstScanMotionRouteProfile::pointing_lissajous:
    return "pointing-lissajous";
  case pipeline::AstScanMotionRouteProfile::rectilinear_continuous_beammap:
    return "rectilinear-continuous-beammap";
  case pipeline::AstScanMotionRouteProfile::unsupported:
    return "unsupported";
  }
  return "unsupported";
}
...
RuntimeConfig load_runtime_config(const fs::path &path) {
  ...
  for (const auto &entry : offsets) {
    ...
    if (!key.starts_with("toltec"))
      continue;
    const auto suffix = key.substr(6);
    require(!suffix.empty() && std::all_of(suffix.begin(), suffix.end(),
                                           [](unsigned char ch) {
                                             return ch >= '0' && ch <= '9';
                                           }),
    ...
    require(std::isfinite(value), "interface sync offset must be finite");
    result.interface_offsets_sec[static_cast<std::size_t>(network)] = value;
    result.interface_offset_present[static_cast<std::size_t>(network)] = true;
  }
  return result;
}
...
fs::path find_raw_file(const fs::path &directory, const SourceBinding &source) {
  const auto &observation = source.header_observation;
  std::ostringstream prefix;
  prefix << source.interface_name << '_' << observation.observation << '_'
         << std::setw(3) << std::setfill('0') << observation.subobservation
         << '_' << std::setw(4) << observation.scan << '_';
...
CadenceEvidence measure_cadence(const pipeline::NativeNetworkAlignment &timing,
  ...
  for (std::size_t index = 1; index < counters.size(); ++index) {
    if (!pipeline::packet_counters_are_contiguous(counters[index - 1],
                                                  counters[index])) {
      continue;
    }
  ...
  if (ordered.size() % 2 == 0) {
    result.median_interval_sec = (ordered[middle - 1] + ordered[middle]) / 2.0;
  }
  const double nominal_interval = 1.0 / sample_frequency_hz;
  for (const double interval : intervals) {
    result.maximum_fractional_deviation =
        std::max(result.maximum_fractional_deviation,
                 std::abs(interval / nominal_interval - 1.0));
  }
  result.uncertainty_within_margin = result.maximum_fractional_deviation <=
                                     maximum_cadence_fractional_uncertainty;
  return result;
}

NetworkInput load_network_input(const fs::path &data_directory,
                                const SourceBinding &source,
                                fixture::Array array,
                                std::size_t detector_count,
                                const RuntimeConfig &config) {
  require(source.network >= 0 && source.network < 13 &&
              config.interface_offset_present[static_cast<std::size_t>(
                  source.network)],
  ...
  require(std::abs(fpga_frequency / accumulation_length / sample_frequency -
  ...
  auto timing = std::make_shared<const pipeline::NativeNetworkAlignment>(
      pipeline::make_native_network_alignment(
          ...
          config.interface_offsets_sec[static_cast<std::size_t>(
              source.network)]));
  ...
  return {source.network,      array,
          detector_count,      std::move(input_file),
          fpga_frequency,      sample_frequency,
          accumulation_length, std::move(timing),
          std::move(runs),     cadence};
}
...
TelescopeEvidence
load_telescope(const fs::path &path,
  ...
  require(read_netcdf_scalar<std::int64_t>(file, "Header.Dcs.ObsNum") ==
                  scope.observation &&
              read_netcdf_scalar<std::int64_t>(file, "Header.Dcs.SubObsNum") ==
                  scope.subobservation &&
              read_netcdf_scalar<std::int64_t>(file, "Header.Dcs.ScanNum") ==
                  scope.scan,
          "telescope producer scope disagrees with APT observation");
  pipeline::AstScanMotionSourceMetadata metadata;
  metadata.producer_kind = pipeline::AstScanMotionProducerKind::real_toltec;
  metadata.dcs_observation_goal = read_netcdf_text(file, "Header.Dcs.ObsGoal");
  ...
  require(count > 1 && count <= static_cast<std::size_t>(
                                    std::numeric_limits<Eigen::Index>::max()),
          "telescope time cardinality is invalid");
  auto times = read_vector(
      file, std::string{pipeline::ast_scan_motion_time_field}, count, "sec");
  auto ra = read_vector(file, std::string{pipeline::ast_scan_motion_ra_field},
                        count, "rad");
  auto dec = read_vector(file, std::string{pipeline::ast_scan_motion_dec_field},
                         count, "rad");
  std::optional<pipeline::AstBeammapMembershipSource> beammap_membership;
  if (metadata.dcs_observation_goal == "BeamMap") {
    metadata.beammap_profile = pipeline::AstBeammapProfileMetadata{
        read_netcdf_scalar<std::int64_t>(file, "Header.Map.ExecMode"),
        read_netcdf_text(file, "Header.Map.MapCoord"),
        read_netcdf_text(file, "Header.Map.MapMotion"),
        read_netcdf_text(file, "Header.Map.MapPath"),
        read_netcdf_scalar<std::int64_t>(file, "Header.Map.HoldDuringTurns"),
        read_netcdf_scalar<double>(file, "Header.Map.XOffset"),
        read_netcdf_scalar<double>(file, "Header.Map.YOffset"),
        read_radian_scalar(file, "Header.Map.XLength"),
        read_radian_scalar(file, "Header.Map.YLength"),
        read_radian_scalar(file, "Header.Map.ScanAngle")};
    beammap_membership.emplace(pipeline::AstBeammapMembershipSource{
        read_integer_vector(
            file, std::string{pipeline::ast_scan_motion_hold_field}, count),
        read_vector(file,
                    std::string{pipeline::ast_scan_motion_tel_az_act_field},
                    count, "rad"),
        read_vector(file,
                    std::string{pipeline::ast_scan_motion_tel_el_act_field},
                    count, "rad"),
        read_vector(file,
                    std::string{pipeline::ast_scan_motion_source_az_field},
                    count, "rad"),
        read_vector(file,
                    std::string{pipeline::ast_scan_motion_source_el_field},
                    count, "rad"),
        read_vector(file,
                    std::string{pipeline::ast_scan_motion_tel_az_cor_field},
                    count, "rad"),
        read_vector(file,
                    std::string{pipeline::ast_scan_motion_tel_el_cor_field},
                    count, "rad")});
  }
  ...
  for (Eigen::Index index = 1; index < times.size(); ++index) {
    ...
    result.minimum_interval_sec = std::min(result.minimum_interval_sec, dt);
    result.maximum_interval_sec = std::max(result.maximum_interval_sec, dt);
  }
  result.source = pipeline::AstScanMotionSource::admit(
      scope, scope, 0, std::move(metadata), std::move(times), std::move(ra),
      std::move(dec), std::move(beammap_membership));
  return result;
}
...
bool same_record(const pipeline::AstScanMotionDerivedRecord &left,
  ...
  return left.causes() == right.causes() &&
         left.physical_scan_member() == right.physical_scan_member() &&
         left.physical_segment_index() == right.physical_segment_index() &&
         ...
         left.realized_direction_valid() == right.realized_direction_valid() &&
...
bool same_summary(const pipeline::AstScanMotionScanSummary &left,
  ...
  return left.maximum_available == right.maximum_available &&
         ...
         left.physical_scan_member_count == right.physical_scan_member_count &&
         left.physical_segment_count == right.physical_segment_count &&
...
ProductEvidence build_product_evidence(
  ...
  const pipeline::AstScanMotionIdentityBinding binding{base + 1U, base + 2U,
                                                       base + 3U, base + 4U};
  auto product = pipeline::build_ast_scan_motion_product(source, binding);
  const auto one_third =
      static_cast<pipeline::AstTelescopeRecord>(source->record_count() / 3);
  const auto two_thirds =
      static_cast<pipeline::AstTelescopeRecord>(2 * source->record_count() / 3);
  ...
  const auto partitioned =
      pipeline::build_ast_scan_motion_product(source, binding, schedule);
  ProductEvidence result;
  result.product = std::move(product);
  for (std::size_t index = 0; index < result.product->record_count(); ++index) {
    result.chunk_record_mismatch_count +=
        !same_record(result.product->record_at_local(index),
                     partitioned->record_at_local(index));
  }
  result.chunk_summary_matches =
      same_summary(result.product->scan_summary(), partitioned->scan_summary());
  return result;
}
...
MappingEvidence map_ast_to_networks(
  ...
  std::vector<std::shared_ptr<const pipeline::NativeNetworkAlignment>> timings;
  for (const auto &input : inputs)
    timings.push_back(input.timing);
  ...
  for (const auto &input : inputs) {
    ...
    motion.mapped_speeds_arcsec_per_sec.assign(
        view.occurrence_count(), std::numeric_limits<double>::quiet_NaN());
    ...
    for (pipeline::TimestreamNativeRow row = view.first_native_row();
      ...
      const auto local =
          static_cast<std::size_t>(row - view.first_native_row());
      ...
      if (local != 0U) {
        ...
        motion.continues_previous[local] =
            pipeline::packet_counters_are_contiguous(counters[local - 1U],
                                                     counters[local])
                ? 1U
                : 0U;
      }
...
std::vector<CadenceGroup>
cadence_groups(const std::vector<NetworkInput> &inputs) {
  ...
  for (const auto &input : inputs) {
    ...
    if (inserted) {
      ...
      position->second.sample_frequency_hz = input.sample_frequency_hz;
    }
...
const NetworkInput &network_input(const std::vector<NetworkInput> &inputs,
                                  std::int64_t network) {
  const auto position =
      std::find_if(inputs.begin(), inputs.end(), [network](const auto &input) {
...
void write_occurrence_admission(std::ostream &output, const NetworkInput &input,
  ...
  const auto admission = fixture::summarize_occurrence_admission(
      motion.mapped_speeds_arcsec_per_sec, motion.continues_previous,
      upper_speed_ceiling_arcsec_per_sec);
  const double primitive_occurrence_duration_sec =
      static_cast<double>(input.accumulation_length) / input.fpga_frequency_hz;
  const auto duration = [&](std::size_t count) {
    return static_cast<double>(count) * primitive_occurrence_duration_sec;
  };
  output << "{\"network\": " << input.network
         << ", \"occurrence_count\": " << admission.occurrence_count
         << ", \"ast_unavailable_count\": " << admission.ast_unavailable_count
         ...
         << ", \"base_admitted_count\": " << admission.base_admitted_count
         ...
         << ", \"retained_run_count\": " << admission.retained_run_count
...
void write_file(std::ostream &output, const FileEvidence &file) {
  output << "{\"role\": \"" << json_escape(file.role) << "\", \"path\": \""
         << json_escape(file.path.string()) << "\", \"sha256\": \""
         << file.sha256 << "\", \"byte_count\": " << file.byte_count << '}';
}

void write_record(const Arguments &arguments, const apt::VerifiedBundle &bundle,
  ...
  require(static_cast<bool>(output), "unable to open fixture census output");
  ...
  output
      << std::setprecision(17) << "{\n"
      << "  \"schema\": \"" << schema << "\",\n"
      << "  \"dataset_id\": \"" << json_escape(arguments.dataset_id) << "\",\n"
      << "  \"source_revision\": \"" << arguments.source_revision << "\",\n"
      << "  \"source_clean_asserted\": "
      << (arguments.source_clean ? "true" : "false") << ",\n"
      << "  \"citlali_version\": \"" << json_escape(CITLALI_GIT_VERSION)
      << "\",\n"
      << "  \"compiler\": \"" << json_escape(__VERSION__) << "\",\n"
      << "  \"cplusplus\": " << __cplusplus << ",\n"
      << "  \"build_identity\": {\n"
      << "    \"executable_sha256\": \"" << executable.sha256 << "\",\n"
      << "    \"netcdf_library\": \"" << json_escape(nc_inq_libvers())
      << "\",\n"
      ...
      << "  },\n"
      << "  \"numerical_policy_id\": \"" << fixture::numerical_policy_id
      << "\",\n"
  ...
  output << "\n  ],\n"
         ...
         << "    \"verified_total_byte_count\": " << bundle.total_byte_count
         << ",\n"
         << "    \"participant_network_count\": " << inputs.size() << ",\n"
         << "    \"detector_count\": " << bundle.apt.rows.size() << ",\n"
         << "    \"root_receipt_sha256\": \""
         << citlali::utils::sha256(bundle.payload.root_receipt_bytes) << "\",\n"
         << "    \"verified_components\": [\n";
  std::size_t component_index = 0;
  for (const auto &[role, bytes] : bundle.payload.component_bytes_by_role) {
    output << "      {\"role\": \"" << json_escape(role) << "\", \"sha256\": \""
           << citlali::utils::sha256(bytes)
           << "\", \"byte_count\": " << bytes.size() << '}';
    if (++component_index != bundle.payload.component_bytes_by_role.size()) {
      output << ',';
    }
    output << '\n';
  }
  output << "    ]\n  },\n"
         << "  \"telescope_ast\": {\n"
         << "    \"policy_id\": \"" << pipeline::ast_scan_motion_policy_id
         << "\",\n"
         << "    \"route_profile\": \""
         << route_profile_token(product.product->route_profile()) << "\",\n"
         << "    \"observation_goal\": \""
         << json_escape(telescope.source->metadata().dcs_observation_goal)
         << "\",\n"
         << "    \"observation_program\": \""
         << json_escape(telescope.source->metadata().dcs_observation_program)
         << "\",\n"
         << "    \"record_count\": " << telescope.source->record_count()
         << ",\n"
         << "    \"physical_scan_member_count\": "
         << summary.physical_scan_member_count << ",\n"
         << "    \"physical_segment_count\": " << summary.physical_segment_count
         << ",\n"
         << "    \"continuity_run_count\": " << summary.continuity_run_count
         << ",\n"
         << "    \"derivative_valid_record_count\": "
         << summary.derivative_valid_record_count << ",\n"
         << "    \"admitted_candidate_count\": "
         << summary.admitted_candidate_count << ",\n"
         << "    \"minimum_interval_sec\": " << telescope.minimum_interval_sec
         << ",\n"
         << "    \"maximum_interval_sec\": " << telescope.maximum_interval_sec
         << ",\n"
         << "    \"maximum_available\": "
  ...
  output << ",\n"
         << "    \"maximizing_record\": " << summary.maximizing_record << ",\n"
         << "    \"telemetry_defect_count\": " << summary.telemetry_defect_count
         << ",\n"
  ...
  for (std::size_t index = 0; index < inputs.size(); ++index) {
    ...
    output << "    {\"network\": " << input.network << ", \"array\": \""
           << fixture::array_name(input.array)
           ...
           << ", \"sample_frequency_hz\": " << input.sample_frequency_hz
           << ", \"fpga_frequency_hz\": " << input.fpga_frequency_hz
           << ", \"accumulation_length\": " << input.accumulation_length
           ...
           << ", \"interval_count\": " << input.cadence.interval_count
           << ", \"minimum_interval_sec\": "
           << input.cadence.minimum_interval_sec
           << ", \"median_interval_sec\": " << input.cadence.median_interval_sec
           ...
           << (input.cadence.uncertainty_within_margin ? "true" : "false")
  ...
  output << "  ],\n"
         ...
         << "    \"missing_support_count\": " << mapping.missing_support_count
         << "\n  },\n"
         << "  \"candidate_mode_domains\": [\n";
  for (std::size_t group_index = 0; group_index < groups.size();
       ++group_index) {
    const auto &group = groups[group_index];
    output << "    {\"array\": \"" << fixture::array_name(group.array)
           << "\", \"sample_frequency_hz\": " << group.sample_frequency_hz
           << ", \"networks\": [";
    ...
    output << "], \"cadence_uncertainty_within_100ppm\": "
           << (group.cadence_uncertainty_within_margin ? "true" : "false")
           << ", \"duration_basis\": "
              "\"occurrence-count-times-accumulation-length-divided-by-fpga-"
              "frequency\""
    ...
    for (int factor = fixture::minimum_factor;
      ...
      output << "      {\"factor\": " << factor << ", \"status\": \"" << status
             << "\", \"ceiling_status\": "
             ...
             << ", \"airy_fwhm_arcsec\": " << evidence.airy_fwhm_arcsec
             ...
             << fixture::constraint_name(evidence.governing_constraint)
      ...
      for (std::size_t network_index = 0; network_index < group.networks.size();
           ++network_index) {
        const auto network = group.networks[network_index];
        output << "        ";
        write_occurrence_admission(output, network_input(inputs, network),
      ...
      output << "}}";
      if (factor != fixture::maximum_factor)
        output << ',';
      output << '\n';
    }
    output << "    ]}";
  ...
  output << "  ],\n"
         ...
         << "}\n";
  require(static_cast<bool>(output), "failed to write fixture census record");
}
...
int main(int argc, char **argv) {
  try {
    ...
    const auto bundle =
        apt::verify_bundle_filesystem(arguments.apt_manifest, true);
    ...
    const pipeline::NativeObservationScope scope{
        observation.observation, observation.subobservation, observation.scan};
    ...
    for (const auto &source : bundle.sources) {
      if (source.role != apt::SourceRole::raw)
        continue;
      sources.push_back({source.network, source.interface_name,
                         source.channel_count, source.content_sha256,
                         source.byte_count, source.header_observation});
    }
    ...
    for (const auto &source : sources) {
      ...
      inputs.push_back(
          load_network_input(arguments.data_directory, source,
    ...
    const auto product =
        build_product_evidence(telescope.source, observation.observation);
    require(product.chunk_record_mismatch_count == 0 &&
                product.chunk_summary_matches,
            "AST product is not chunk-partition invariant");
    const auto mapping = map_ast_to_networks(scope, product.product, inputs);
    ...
    write_record(arguments, bundle, matched_relation_available,
                 file_evidence("effective_config", arguments.config), telescope,
                 auxiliary, inputs, product, mapping, cadence_groups(inputs));
    return 0;
  } catch (const std::exception &error) {
    std::cerr << "WP-7 RTC filter fixture census failed: " << error.what()
              << '\n';
    return 1;
  }
}
