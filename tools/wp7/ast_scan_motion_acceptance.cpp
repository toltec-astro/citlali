...
constexpr std::string_view acceptance_schema =
    "citlali-wp7-ast-scan-motion-acceptance-v2";
...
std::string usage() {
  return "Usage: citlali_wp7_ast_scan_motion_acceptance\n"
...
Arguments parse_arguments(int argc, char **argv) {
  ...
  require(!result.source_revision.empty(), "--source-revision is required");
  return result;
}
...
NetworkInput
load_network_input(const fs::path &data_directory,
                   const pipeline::CanonicalAptRawSourceBindingV2 &source,
  ...
  auto timing = std::make_shared<const pipeline::NativeNetworkAlignment>(
      pipeline::make_native_network_alignment(
          ...
          config.interface_offsets_sec[static_cast<std::size_t>(
              source.network)]));
  return {source.network, path.filename().string(), digest, byte_count,
          std::move(timing)};
}
...
double great_circle_angle(const std::array<double, 3> &left,
  ...
  const std::array<double, 3> cross{left[1] * right[2] - left[2] * right[1],
                                    left[2] * right[0] - left[0] * right[2],
                                    left[0] * right[1] - left[1] * right[0]};
  const double cross_norm =
      std::hypot(std::hypot(cross[0], cross[1]), cross[2]);
  const double dot = std::clamp(
      left[0] * right[0] + left[1] * right[1] + left[2] * right[2], -1.0, 1.0);
...
TelescopeLoad load_telescope(const fs::path &path,
  ...
  const auto digest = citlali::utils::sha256_file(path);
  require(digest == telescope_sha256, "telescope SHA-256 is not approved");
  ...
  require(read_netcdf_scalar<int>(file, "Header.Dcs.ObsNum") ==
              ...
              read_netcdf_scalar<int>(file, "Header.Dcs.ScanNum") == scope.scan,
  ...
  auto times =
      read_vector(file, std::string{pipeline::ast_scan_motion_time_field},
  ...
  for (Eigen::Index index = 1; index < times.size(); ++index) {
    ...
    const double speed =
        great_circle_angle(direction(ra(index - 1), dec(index - 1)),
...
ProductEvidence build_product_evidence(
  ...
  auto product =
      pipeline::build_ast_scan_motion_product(source, identity_binding);
  ...
  for (std::size_t index = 0; index < product->record_count(); ++index) {
    ...
    evidence.quality_classified_count += record.telemetry_quality_classified();
  ...
  for (std::size_t index = 0; index < product->record_count(); ++index) {
    evidence.record_mismatch_count += !same_record(
        product->record_at_local(index), partitioned->record_at_local(index));
  ...
  evidence.summary_mismatch_count =
      !same_summary(product->scan_summary(), partitioned->scan_summary());
  return evidence;
}
...
MappingEvidence build_mapping_evidence(
  ...
  for (const auto &input : inputs) {
    ...
    for (std::size_t index = 1; index < counters.size(); ++index) {
      network.packet_discontinuity_count +=
          !pipeline::packet_counters_are_contiguous(counters[index - 1],
                                                    counters[index]);
    }
    for (pipeline::TimestreamNativeRow row = view.first_native_row();
      ...
      if (!record.available()) {
        ...
        network.telemetry_defect_cause_count += pipeline::has_cause(
            record.causes(), pipeline::AstScanMotionCause::telemetry_defect);
        evidence.missing_unavailable_cause_count += !pipeline::has_cause(
            record.causes(),
      ...
      const double expected_speed =
          support->lower_weight *
              product->record_at_local(lower).scalar_speed_arcsec_per_sec() +
          support->upper_weight *
              product->record_at_local(upper).scalar_speed_arcsec_per_sec();
      ...
      const bool support_valid =
          ...
          support->lower_weight >= 0.0 && support->upper_weight >= 0.0 &&
          support->lower_weight <= 1.0 && support->upper_weight <= 1.0 &&
          std::abs((support->lower_weight + support->upper_weight) - 1.0) <=
    ...
    evidence.mapped_owned_bytes += view.memory_evidence().logical_owned_bytes();
  ...
  evidence.cpu_time_sec =
      static_cast<double>(cpu_end - cpu_begin) / CLOCKS_PER_SEC;
  const auto nw0 =
      std::find_if(evidence.networks.begin(), evidence.networks.end(),
  ...
  if (nw0 != evidence.networks.end() && nw7 != evidence.networks.end()) {
    ...
    evidence.nw0_nw7_distinct = bit_pattern(evidence.nw0_first_time_unix_sec) !=
...
void write_record(const Arguments &arguments,
                  ...
                  const MappingEvidence &mapping, std::uint64_t peak_rss) {
  std::ofstream output(arguments.output);
  require(static_cast<bool>(output), "unable to open AST acceptance output");
  ...
  output << std::setprecision(17) << "{\n"
         ...
         << "  \"executable_revision\": \"" << CITLALI_WP7_SOURCE_REVISION
         << "\",\n"
         << "  \"executable_version\": \"" << json_escape(CITLALI_GIT_VERSION)
         << "\",\n"
         ...
         << "  \"kidscpp_revision\": \"" << CITLALI_WP7_KIDSCPP_REVISION
         << "\",\n"
         << "  \"kidscpp_build_patch_sha256\": \""
         << CITLALI_WP7_KIDSCPP_PATCH_SHA256 << "\",\n"
         << "  \"kidscpp_tree\": \"" << CITLALI_WP7_KIDSCPP_TREE << "\",\n"
         << "  \"tula_revision\": \"" << CITLALI_WP7_TULA_REVISION << "\",\n"
         << "  \"tula_build_patch_sha256\": \"" << CITLALI_WP7_TULA_PATCH_SHA256
         << "\",\n"
         << "  \"tula_tree\": \"" << CITLALI_WP7_TULA_TREE << "\",\n"
         ...
         << "  \"product_role\": \"" << pipeline::ast_scan_motion_product_role
         << "\",\n"
         << "  \"representative_dataset_id\": \"" << representative_dataset_id
         << "\",\n"
         << "  \"observation\": 152390,\n"
         << "  \"subobservation\": 0,\n"
         << "  \"scan\": 2,\n"
         ...
         << ",\n"
         << "    \"time_field\": \"" << pipeline::ast_scan_motion_time_field
         << "\",\n"
         << "    \"ra_field\": \"" << pipeline::ast_scan_motion_ra_field
         << "\",\n"
         << "    \"dec_field\": \"" << pipeline::ast_scan_motion_dec_field
         << "\",\n"
         ...
         << "    \"participant_network_count\": " << mapping.networks.size()
         << "\n"
         << "  },\n"
         << "  \"identity_binding\": {\n"
         ...
         << "    \"telemetry_defect_count\": " << product.defect_records.size()
         << ",\n"
         << "    \"telemetry_defect_records\": [";
  for (std::size_t index = 0; index < product.defect_records.size(); ++index) {
    if (index != 0)
      output << ", ";
  ...
  output << "],\n"
         ...
         << "    \"derivative_valid_count\": " << product.derivative_valid_count
         << ",\n"
         << "    \"maximum_available\": "
         ...
         << "    \"derived_record_bytes\": " << memory.derived_record_bytes
         << ",\n"
         ...
         << "    \"referenced_beammap_membership_plane_count\": "
         << memory.referenced_beammap_membership_plane_count << ",\n"
         << "    \"physical_segment_directory_bytes\": "
         << memory.physical_segment_directory_bytes << "\n"
         << "  },\n"
         << "  \"chunk_invariance\": {\n"
         << "    \"partition_count\": 3,\n"
         << "    \"record_mismatch_count\": " << product.record_mismatch_count
         << ",\n"
         ...
         << "    \"summary_mismatch_count\": " << product.summary_mismatch_count
         << "\n"
         << "  },\n"
         << "  \"network_mapping\": {\n"
         << "    \"timing_scope\": \"network-specific\",\n"
         << "    \"total_occurrence_count\": " << mapping.total_occurrence_count
         << ",\n"
         ...
         << "    \"support_mismatch_count\": " << mapping.support_mismatch_count
         << ",\n"
         << "    \"value_mismatch_count\": " << mapping.value_mismatch_count
         << ",\n"
         << "    \"missing_unavailable_cause_count\": "
         << mapping.missing_unavailable_cause_count << ",\n"
         << "    \"mapped_owned_bytes\": " << mapping.mapped_owned_bytes
         << ",\n"
  ...
  for (std::size_t index = 0; index < mapping.networks.size(); ++index) {
    ...
    output << "      {\"network\": " << network.network << ", \"filename\": \""
           << json_escape(network.filename) << "\", \"sha256\": \""
           << network.sha256 << "\", \"byte_count\": " << network.byte_count
           ...
           << ", \"first_time_unix_sec\": " << network.first_time_unix_sec
           << ", \"last_time_unix_sec\": " << network.last_time_unix_sec << "}";
    if (index + 1 != mapping.networks.size())
      output << ',';
    output << '\n';
  }
  output << "    ]\n"
         << "  },\n"
         << "  \"performance\": {\n"
         << "    \"raw_product_wall_time_sec\": " << product.wall_time_sec
         << ",\n"
         << "    \"raw_product_cpu_time_sec\": " << product.cpu_time_sec
         << ",\n"
         << "    \"network_mapping_wall_time_sec\": " << mapping.wall_time_sec
         << ",\n"
         << "    \"network_mapping_cpu_time_sec\": " << mapping.cpu_time_sec
         << ",\n"
         << "    \"process_peak_rss_bytes\": " << peak_rss << "\n"
         << "  },\n"
         << "  \"unexpected_error_count\": 0\n"
         << "}\n";
  require(static_cast<bool>(output), "failed to write AST acceptance record");
}
...
int main(int argc, char **argv) {
  try {
    ...
    require(arguments.design_is_ancestor && arguments.align_repair_is_ancestor,
    ...
    require(observation.observation == 152390 &&
                observation.subobservation == 0 && observation.scan == 2,
    ...
    for (const auto &source : relation.raw_sources()) {
      inputs.push_back(
          load_network_input(arguments.data_directory, source, config));
    }
    ...
    require(product.product->scan_summary().maximum_available &&
                product.product->scan_summary().maximum_speed_arcsec_per_sec >
                    200.0 &&
                product.product->scan_summary().maximum_speed_arcsec_per_sec <
                    230.0,
    ...
    const auto mapping = build_mapping_evidence(scope, product.product, inputs);
    ...
    write_record(arguments, relation, telescope, product, mapping,
                 peak_rss_bytes());
    std::cout << "WP-7 AST scan-motion acceptance record: " << arguments.output
              << '\n';
    return 0;
